...
if nargin<1,
    ...
    if ~isfield(CONN_x,'folders'),CONN_x.folders=struct('rois',[],'data',[],'preprocessing',[],'firstlevel',[],'firstlevel_vv','firstlevel_dyn','secondlevel',[]);end
    if CONN_x.pobj.holdsdata
        ...
        if conn('checkver','15.h',CONN_x.ver)
            CONN_x.folders.firstlevel_vv=fullfile(path,name,'results','firstlevel','voxel2voxel'); [ok,nill]=mkdir(path,name);[ok,nill]=mkdir(fullfile(path,name),'results'); [ok,nill]=mkdir(fullfile(path,name,'results'),'firstlevel'); [ok,nill]=mkdir(fullfile(path,name,'results','firstlevel'),'voxel2voxel');
            CONN_x.folders.firstlevel_dyn=fullfile(path,name,'results','firstlevel','dynfc'); [ok,nill]=mkdir(path,name);[ok,nill]=mkdir(fullfile(path,name),'results'); [ok,nill]=mkdir(fullfile(path,name,'results'),'firstlevel'); [ok,nill]=mkdir(fullfile(path,name,'results','firstlevel'),'dynfc');
        else
    ...
    if ~isfield(CONN_x,'dynAnalyses'), CONN_x.dynAnalyses=struct('regressors', struct('names',{{}}),'variables', struct('names',{{}}),'Ncomponents',[20],'condition',[1],'window',10,'output',[1 1 0]); end
    if ~isfield(CONN_x.dynAnalyses,'condition')||isempty(CONN_x.dynAnalyses.condition), CONN_x.dynAnalyses.condition=1; end
    if ~isfield(CONN_x.dynAnalyses,'window')||isempty(CONN_x.dynAnalyses.window), 
        if isfield(CONN_x.dynAnalyses,'filter')&&~isempty(CONN_x.dynAnalyses.filter), CONN_x.dynAnalyses.window=round(1/CONN_x.dynAnalyses.filter);
        else CONN_x.dynAnalyses.window=10;
        end
    end
