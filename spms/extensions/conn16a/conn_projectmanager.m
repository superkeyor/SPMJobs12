...
switch(lower(option))
    case 'null'
        ...
        pobj.id='';
        pobj.holdsdata=true;      % has this project an associated data folder?
    ...
    case 'extendedname', % checks if input filename indicates extended project
        filename=varargin{1};
        [jid,jidx]=regexp(filename,'\?id=(\d+)\s*(,\s*version\s*=[^,\?]+)?(,\s*subjects\s*=[^\?]+)?\s*$','tokens','start','once');
        if ~isempty(jid),
                    ...
                    if exist(pathname,'dir')&&conn_existfile(fullfile(pathname,'info.mat'))
                        ...
                        if ~all(ismember(info.tagmsg,validlabels)),
                            if dogui&&(isequal(CONN_x.gui,1)||(isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'display')&&CONN_x.gui.display))
                                answ=questdlg({'Warning: There are pending jobs submitted but not yet finished','','Changes to this project will be disregarded until all pending jobs are finished or canceled', '', 'Do you want to see these pending jobs now?'},'Warning!','Yes','No','Yes');
