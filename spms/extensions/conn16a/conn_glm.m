function [h,F,p,dof,statsname,BB,EE,opt]=conn_glm(X,Y,C,M,opt,dop)
...
if Ns==1&&isequal(M,1) %(Ns=1,M=1)
    ...
    if univariate
        ...
        for na=nas(:)'
            ...
            if Na>1
                if isempty(Na_h), Na_h=nan(size(h,1),size(h,2),Na);Na_F=nan(size(F,1),size(F,2),Na); end
                Na_h(:,:,na)=h;
                Na_F(:,:,na)=F;
            end
        end
