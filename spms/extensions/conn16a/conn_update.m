function updated=conn_update(ver,force,silent)
global CONN_x

if nargin<1||isempty(ver), ver=conn('ver'); end
...
updated=false;
if ~isempty(ver),
    version_conn=regexp(ver,'(\d.*?)\.(\w+)','tokens','once');
...
if ~ok, if ~silent, conn_update_disp(['Cannot access ',[url0,url1],' to check for updates. Please try again later']); end
else
    ...
    if isempty(version_url), 
        if ~silent, conn_update_disp('Sorry, automatic updates not available. Please check back later'); end
        return;
    end
    if force||existprev,
        if existprev, 
            if iscell(version_url) && length(version_url)==2 && (str2num(version_url{1})<str2num(version_conn{1}) || (str2num(version_url{1})==str2num(version_conn{1}) && base2dec(version_url{2},36)<=base2dec(version_conn{2},36))), samever=1; else,samever=0;end
        elseif force,samever=0;
        end
        if samever,
            if ~silent, conn_update_disp({['Your version is up to date']}); end
        else
            if 0,% only check / do not offer to download 
                ...
                if strcmp(answ,'Yes'),
                    ...
                    if ~isempty(hfig)&&ishandle(hfig)&&~silent,
                        answ=questdlg({'Warning. Your current CONN session needs to be closed to proceed with this update. Any unsaved data will be lost','Do you want to continue?'},'conn update','Yes', 'No', 'Yes');
                    else, answ='Yes';end
                    if strcmp(answ,'Yes'),
...
%                         ok=0;
                        while ~ok,
                            if force==2, answ=input(['Installation path of conn directory ' path1 '? (Y/N) : '],'s'); if ~isequal(answ,'Y'), path1=input(['Installation path of conn directory: '],'s'); end
                            else path1=uigetdir(path1,'Select a directory to install the new version');
                            end
                            if isequal(path1,0),break;end
                            [ok,ko]=mkdir(path1,'conn');
                            %[ok,ko]=mkdir(fileparts(path1),['conn',version_url{1},version_url{2}]);
                            if ~ok,
                                conn_update_disp(['Unable to create directory ',path1,' - check permissions']);
                            end
                        end
                        if ~isequal(path1,0),
                            [ok,ko]=mkdir(path1,'conn');
                            ...
                            conn_update_disp({['CONN successfully updated (release ',version_url{1},'.',version_url{2},')'],['Installation folder ',path2]});
                                ...
                                if ~isequal(path0,path2)
                                    ...
                                    if force==2, conn_update_disp({['Update (',version_url{1},'.',version_url{2},') successfully installed'],['Warning: MATLAB path not yet saved for future sessions.'],['To use this new release in future MATLAB sessions type:'],['   addpath ',path2]});
                                        ...
                                        if ~isequal(outputpathfilename,0)
                                            ...
                                            if ko, conn_update_disp({['Update (',version_url{1},'.',version_url{2},') successfully installed'],['Warning: Not possible to save MATLAB path. Check file permissions.'],['To use this new release in future MATLAB sessions type:'],' ',['   addpath ',path2]});
                                            else, conn_update_disp({['Update (',version_url{1},'.',version_url{2},') successfully installed'],['Saved new MATLAB path: ',path2],['You can delete if you wish the previous version at ',path0]}); end
                                        else
                                            conn_update_disp({['Update (',version_url{1},'.',version_url{2},') successfully installed'],['You may delete if you wish the previous version at ',path0],'Warning: MATLAB path not saved',['To use this new release in future MATLAB sessions type:'],' ',['   addpath ',path2]});
                                        end
                                    end
                                end
                            end
                        else conn_update_disp(['Installation cancelled']);
                        end
        ...
        conn_update_disp({['CONN version ',version_url{1},'.',version_url{2},' available at ',url0,url1]});
    end
end
    ...
    function conn_update_disp(varargin)
        ...
        if isfield(CONN_x,'gui')&&~isstruct(CONN_x.gui)&&CONN_x.gui,
            ...
            if ~isempty(varargin), h=conn_msgbox(varargin{1},'conn_update'); end
        else,
            if ~isempty(varargin), disp(char(varargin{:})); end
        end
