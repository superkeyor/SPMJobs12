...
switch(lower(option))
    case {'measures','empty','measurenames'}
        measures=struct(...
            'names',{{'group-PCA','group-ICA','group-MVPA','IntegratedLocalCorrelation','RadialCorrelationContrast','IntrinsicConnectivityContrast','RadalSimilarityContrast'}},... % connectome-level Multivariate Pattern Analysis; Integrated Local Correlation; Radial Correlation Contrast; Intrinsic Connectivity Contrast; Radial Similarity Contrast; 
            'measuretype',{{4,3,2,1,1,1,1}},...
            'localsupport',{{nan,nan,nan,8,8,0,0}},...
            'global',{{nan,nan,nan,0,0,1,1}},...
            'deriv',{{nan,nan,nan,0,1,0,1}},...
            'norm',{{nan,nan,1,1,1,1,1}},...
            'filename',{{'','','','','','',''}},...
            'dimensions_out',{{20,20,4,1,3,1,3}},...
            'dimensions_in',{{64,64,64,64,64,64,64}});
        ...
        for index=1:numel(options.names)
            ...
            %for ncomp=1-(options.dimensions_out{index}>1):options.dimensions_out{index}
                ...
                for n1=1:numel(optionsnames),
                    ...
                    else measurename{end}=[measurename{end},'#<',num2str(options.(optionsnames{n1}){index}),'#>'];
                    end
                end
    ...
    case 'match_existing'
        ...
        idx=find(strncmp([measurename,'#<'],CONN_x.vvAnalyses.measures,numel(measurename)+2));
        if ~isempty(idx),
            imeasure=idx;
            isnew=0;
        else
    ...
    case 'match_extended'
        ...
        idx=strmatch(measurename,CONN_x.vvAnalyses.measurenames,'exact');
        if ~isempty(idx),
            imeasure=idx(1);
            isnew=0;
        else
    ...
    case 'match_measures'
        ...
        for n1=1:numel(optionsnames),
            ...
            else measurename=[measurename,'#<',num2str(options.(optionsnames{n1}){index}),'#>'];
            end
        end
        if nargin<3, flag='-'; else flag=varargin{3}; end
        if ~isfield(CONN_x.vvAnalyses,'measurenames'),
            CONN_x.vvAnalyses.measurenames={};
        end
        idx=strmatch(measurename,CONN_x.vvAnalyses.measurenames,'exact');
        if ~isempty(idx),
            imeasure=idx(1);
            ...
            imeasure=length(CONN_x.vvAnalyses.measurenames)+1;
            if flag=='+',
                CONN_x.vvAnalyses.measurenames{imeasure}=measurename;
    ...
    case 'compute_step'
        ...
        elseif all(isinf(options.localsupport)), y=mean(x(:));
        else
            ok=0;
...
%         end
    ...
    case 'compute_slice'
        ...
        if ~ok
            %hw=msgbox('Loading subject data. Please wait');
            %[ndgridx,ndgridy,ndgridz]=ndgrid(1:vols.matdim.dim(1),1:vols.matdim.dim(2),1:vols.matdim.dim(3));
            %xyz=[ndgridx(:),ndgridy(:),ndgridz(:),ones(numel(ndgridx),1)]';
            x=zeros([vols.size.Nt,vols.matdim.dim]);
            for n1=1:vols.size.Nt, x(n1,:)=reshape(conn_get_time(vols,n1),1,[]); end
            %x=reshape(spm_get_data(vols,xyz),[numel(vols),vols(1).dim(1:3)]);
        ...
        elseif all(isinf(options.localsupport)), 
            ...
            if options.deriv, y=repmat(y,[1,1,1,3]); end
        else
