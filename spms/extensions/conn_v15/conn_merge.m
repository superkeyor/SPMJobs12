...
if nargin>0&&ischar(value0), % merge multiple project files (value0 = filenames)
    ...
    elseif numel(value)==1&&value>=value0, idx1=1:value;
        ...
        Ransw=questdlg('This will permanently remove any connectivity data for the selected subjects. Are you sure you want to continue?','','Yes','No','No');
...
if mergeinfo
    ...
    for n1=1:length(other), % consistency check for source/measure/condition _list info
        ...
        if isfield(CONN_x,'vvAnalyses')&&isfield(other{n1}.CONN_x,'vvAnalyses')
            if isfield(CONN_x.vvAnalyses,'measurenames')&&isfield(other{n1}.CONN_x.vvAnalyses,'measurenames')
                measurenames1=CONN_x.vvAnalyses.measurenames;
                measurenames2=other{n1}.CONN_x.vvAnalyses.measurenames;
                ...
                if numel(measurenames2)>numel(measurenames1), CONN_x.vvAnalyses.measurenames=measurenames2; end
            end
        end
    ...
    for nsub=setdiff(idx1,idx0), % new subjects only
        ...
        try, CONN_x.Setup.roiextract_functional{nsub}=other{copyfromproject}.CONN_x.Setup.roiextract_functional{copyfromsubject};
        end
        try, CONN_x.Setup.unwarp_functional{nsub}=other{copyfromproject}.CONN_x.Setup.unwarp_functional{copyfromsubject};
        end
        ...
        CONN_x.Setup.nsessions(nsub)=other{copyfromproject}.CONN_x.Setup.nsessions(copyfromsubject);
        CONN_x.Setup.RT(nsub)=other{copyfromproject}.CONN_x.Setup.RT(copyfromsubject);
    end
    ...
    try, CONN_x.Setup.roiextract_functional={CONN_x.Setup.roiextract_functional{tempidx1}};
    end
    ...
    CONN_x.Setup.nsessions=CONN_x.Setup.nsessions(tempidx1);
    CONN_x.Setup.RT=CONN_x.Setup.RT(tempidx1);
end
...
if copyfiles && (value<value0||any(otheridx3(setdiff(idx1,idx0)))) && ~isempty(CONN_x.filename),
    ...
    for nother=1:length(other),
        ...
        filepathresults2=other{nother}.CONN_x.folders.firstlevel;
        ...
        filepathall{nother}={filepath,filepathresults1,filepathresults2,filepathresults3{:}};
            ...
            for n1=1:length(files),
                ...
                if ~isempty(k),
                    ...
                    if ~isempty(m)&&length(str2num(m))==1,
                        ...
                        filenamesall{nother}=strvcat(filenamesall{nother},fullfile(filepathall{nother}{n0},files(n1).name));
                    end
                end
            end
        end
