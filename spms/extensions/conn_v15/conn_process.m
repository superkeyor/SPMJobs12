...
if ischar(options),
    ...
    while ~isempty(optionsnow),
        switch(lower(optionsnow)),
            ...
            case 'all',             disp(['CONN: RUNNING ALL ANALYSIS STEPS']); conn_process([0:4,4.5,5,1.5,2,[6:11 13:15],20]);
            ...
            case 'analyses_seed',   disp(['CONN: RUNNING ANALYSIS STEP (ROI-to-ROI or seed-to-voxel analyses)']); conn_process([9,10],varargin{:});
            case 'analyses_vv',     disp(['CONN: RUNNING ANALYSIS STEP (voxel-to-voxel analyses)']); conn_process([9,13],varargin{:});
            case 'analyses_vvmvpa',     disp(['CONN: RUNNING ANALYSIS STEP (voxel-to-voxel MVPA analyses)']); conn_process([9,13.5],varargin{:});
            case 'analyses_roi',    disp(['CONN: RUNNING ANALYSIS STEP (roi-based analyses only)']); conn_process([9,11,15],varargin{:});
            case 'analyses_seedandroi', disp(['CONN: RUNNING ANALYSIS STEP (roi-based analyses only)']); conn_process([9:11,15],varargin{:});
            case 'analyses_dyn',    disp(['CONN: RUNNING DYNAMIC CONNECTIVITY STEP']); conn_process([9,20],varargin{:});
            ...
            case 'analyses_gui_vvmvpa', disp(['CONN: RUNNING ANALYSIS STEP (voxel-to-voxel MVPA analyses)']); conn_process([13.5],varargin{:});
            case 'analyses_gui_dyn',disp(['CONN: RUNNING DYNAMIC CONNECTIVITY STEP']); conn_process([20],varargin{:});
            case 'extract_connectome',conn_process(14,varargin{:});
	...
	for nsub=validsubjects,
        ...
        for nroi=4:nrois,%1:nrois,
            ...
            for nses=1:nsesstemp,
                ...
                elseif isstruct(CONN_x.Setup.rois.files{nsubstemp}{nroi}{nses}{3})
                    try
		...
		for nses=1:nsess,
            ...
            elseif length(CONN_x.Setup.nscans{nsub})<nses, ERR{end+1}=['ERROR: Subject ',num2str(nsub),' has no functional data defined for session ',num2str(nses)]; 
                ...
                elseif nscans<2, ERR{end+1}=['ERROR: Subject ',num2str(nsub),' Session ',num2str(nses), ' functional data has ',num2str(nscans),' time-points (scans) defined']; 
                end
        ...
        if any(~okconditions(validconditions))
            for ncondition=validconditions(~okconditions(validconditions));
                ...
                else ERR{end+1}=['ERROR: Subject ',num2str(nsub),' does not have any scan associated with condition ',CONN_x.Setup.conditions.names{ncondition}];
                end
            end
        end
    end
		...
		for nses=1:nsess,
            ...
            if strcmp(lower(REDO),'yes')||isempty(dir(filename)),
                ...
                for ncondition=1:nconditions,
                    ...
                    for n1=1:length(onset), x(100+round(1+onset(n1)/rt+(0:durat(min(length(durat),n1))/rt-1)))=1; end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if any(options==4) && any(CONN_x.Setup.steps([1,2,3])) && ~(isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'steps')&&~any(CONN_x.gui.steps([1,2,3]))),
	[path,name,ext]=fileparts(CONN_x.filename);
    filepath=CONN_x.folders.data;
    if isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'subjects'), validsubjects=CONN_x.gui.subjects; else validsubjects=1:CONN_x.Setup.nsubjects; end
    if isfield(CONN_x,'pobj')&&isstruct(CONN_x.pobj)&&isfield(CONN_x.pobj,'subjects'), validsubjects=CONN_x.pobj.subjects; end
	...
	for nsub=validsubjects,
        ...
        if isfield(CONN_x.Setup,'cwthreshold')&&numel(CONN_x.Setup.cwthreshold)>0, THR=CONN_x.Setup.cwthreshold(1);
        else THR=.5;  % threshold for white/CSF probability maps before erosion
        end
        if isfield(CONN_x.Setup,'cwthreshold')&&numel(CONN_x.Setup.cwthreshold)>1, ERODE=CONN_x.Setup.cwthreshold(2);
        else ERODE=1; % erosing level for white/csf masks (voxels) (set to 0 for no erosion)
        end
        ...
        if ERODE>0,
            for nroi=intersect(validrois,2:3),
                ...
                for nses=1:nsesstemp
                    ...
                    if (~SKIPRECOMPUTE&&strcmp(lower(REDO),'yes'))||~conn_existfile(Vmask{nroi}{nses}),
                        [nill,nill,ext]=fileparts(Vmask{nroi}{nses});
                        switch(ext),
                            case {'.img','.nii','.hdr'},
                                ...
                                for n1=1:length(idxt), if (sum(sum(sum(X0(idxx(idxt(n1))+(-ERODE:ERODE),idxy(idxt(n1))+(-ERODE:ERODE),idxz(idxt(n1))+(-ERODE:ERODE))<THR,3),2),1))>1, idxt(n1)=0; end; end
                                idxt=idxt(idxt>0);
                                idx1=idx1(idxt);
                                X1=zeros(size(X0));X1(idx1)=1;
                                V0.fname=conn_prepend('e',CONN_x.Setup.rois.files{nsub}{nroi}{nses}{1});spm_write_vol(V0,X1);
                                if ~nnz(X1), error(sprintf('No suprathreshold voxels in ROI file %s after erosion (original file contained %d suprathreshold voxels)',CONN_x.Setup.rois.files{nsub}{nroi}{nses}{1},nnz(X0>THR)));
                                end
                            otherwise,
		...
		for nses=1:nsess,
            ...
            try
                ...
                else VsourceUnsmoothed=char(VsourceUnsmoothed);
                end
            catch
                VsourceUnsmoothed=Vsource;
            ...
            if any(validrois==1)
                if strcmp(lower(REDO),'yes')||isempty(old)||~isfield(old,'sampledata')||~isfield(old,'samplexyz'),
                    [sampledata,samplexyz]=conn_rex(Vsource,Vmask{1}{min(nses,numel(Vmask{1}))},'dims',256,'level','subsetvoxels','scaling',scalinglevel,'select_clusters',0,'output_type','none');
                else, sampledata=old.sampledata; samplexyz=old.samplexyz; end
            ...
            for nroi=validrois,
                    ...
                    if CONN_x.Setup.rois.multiplelabels(nroi), 
                        ...
                        if isempty(idx) failed=1;
                            ...
                            if isempty(dir(fullfile(roi_path_dir,[roi_path_name,'.txt']))),
                                ...
                                [nill,tnames]=conn_rex(Vmask{nroi}{min(nses,nsesstemp)},Vmask{nroi}{min(nses,nsesstemp)},'summary_measure','mean','conjunction_mask',mask,'level',level,'scaling',scalinglevel,'select_clusters',0,'covariates',entercovariates,'output_type','none');
                ...
                if strcmp(lower(REDO),'yes')||isempty(old)||failed,
                    ...
                    if CONN_x.Setup.rois.unsmoothedvolumes(nroi), Vsourcethis=VsourceUnsmoothed; else, Vsourcethis=Vsource; end
                    if isfield(CONN_x.Setup,'outputfiles')&&numel(CONN_x.Setup.outputfiles)>=6&&CONN_x.Setup.outputfiles(6), outputtype='saverex'; filenamerex=fullfile(filepath,['REX_Subject',num2str(nsub,'%03d'),'_Session',num2str(nses,'%03d'),'_ROI',num2str(nroi),'.mat']);
...
%                             if ~isempty(fsanatomical)&&~isempty(dir(fullfile(roi_path_dir,[roi_path_name,'.txt'])))&&~isempty(dir(fullfile(filepath,['DATA_Subject',num2str(nsub,'%03d'),'_Session',num2str(nses,'%03d'),'.mat']))),
%                                 vdata=conn_vol(fullfile(filepath,['DATA_Subject',num2str(nsub,'%03d'),'_Session',num2str(nses,'%03d'),'.mat']));
%                                 if conn_surf_dimscheck(vdata.matdim.dim)
%                                     tnames=textread(fullfile(roi_path_dir,[roi_path_name,'.txt']),'%s','delimiter','\n');
%                                     tvol=spm_vol(Vmask{nroi});
%                                     tdata=spm_read_vols(tvol);
%                                     tdata=reshape(tdata,vdata.matdim.dim);
%                                     maxtdata=max(tdata(:));
%                                     data=repmat({0},1,maxtdata);
%                                     datan=zeros(1,maxtdata);
%                                     namesroi={};
%                                     for nslice=1:vdata.size.Ns
%                                         [tx,tidx]=conn_get_slice(vdata,nslice);
%                                         troi=tdata(tidx+(nslice-1)*vdata.matdim.dim(1)*vdata.matdim.dim(2));
%                                         for n=find(troi(:)'>0)
%                                             data{troi(n)}=data{troi(n)}+tx(:,n);
%                                             datan(troi(n))=datan(troi(n))+1;
%                                         end
%                                     end
%                                     for n=1:numel(data), data{n}=data{n}/max(eps,datan(n)); end
%                                     namesroi=tnames;
%                                     urois=unique(tdata(tdata>0));
%                                     data=cell2mat(data(urois));
%                                     namesroi=cellfun(@(x)[CONN_x.Setup.rois.names{nroi},'.',x],namesroi(urois)','uni',0);
%                                     params=[];
%                                     tdone=true;
%                                 end
%                             end
%                             if ~tdone, [data{nroi1},namesroi{nroi},params]=conn_rex(Vsourcethis,Vmask{nroi},'summary_measure','mean','conjunction_mask',mask,'level',level,'scaling',scalinglevel,'select_clusters',0,'covariates',entercovariates,'fsanatomical',fsanatomical,'output_type','saverex','output_folder',filepath); end
                        end
                    end
                    ...
                    if CONN_x.Setup.rois.multiplelabels(nroi),
                        ...
                        for n1=1:size(datatemp,2),
                            data{nroi1}=datatemp(:,n1); 
                            names{nroi1}=namesroi{nroi}{n1};
                            if any(strcmp(namesroi{nroi}{n1},names(1:nroi1-1))), error('duplicated ROI name %s',namesroi{nroi}{n1}); end
...
if any(options==4.5) && any(CONN_x.Setup.steps([1,2,3])) && ~(isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'steps')&&~any(CONN_x.gui.steps([1,2,3]))),
	...
	for nsub=validsubjects,
        ...
        if strcmp(lower(REDO),'yes')||any(missingdata),
            ...
            for ncondition=validconditions,
                ...
                if crop||numel(CONN_x.Setup.conditions.filter{ncondition})==2, softlink=[]; softlinkoverwrite=false; 
                else softlink=['DATA_Subject',num2str(nsub,'%03d'),'_Condition',num2str(0,'%03d'),'.mat']; 
                    softlinkoverwrite=~softdone|~conn_existfile(fullfile(filepathresults,softlink)); % avoids data duplication
                    softdone=true;
                end
                Yout{ncondition}=conn_init_vol(Yout{ncondition},[],[],softlink,softlinkoverwrite);
            end
            B=[];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if any(options==7) && any(CONN_x.Setup.steps([1,2])) && ~(isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'steps')&&~any(CONN_x.gui.steps([1,2]))),
	[path,name,ext]=fileparts(CONN_x.filename);
    filepath=CONN_x.folders.data;
	filepathresults=CONN_x.folders.preprocessing;
    if isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'subjects'), validsubjects=CONN_x.gui.subjects; else validsubjects=1:CONN_x.Setup.nsubjects; end
    ...
    REDO='Yes';%filename=fullfile(filepathresults,['ROI_Subject',num2str(1,'%03d'),'_Condition',num2str(icondition(validconditions(1)),'%03d'),'.mat']);
    %if ~isempty(dir(filename)),if isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'overwrite'), REDO=CONN_x.gui.overwrite; else, REDO=questdlg('Overwrite existing subject results?','','Yes', 'No', 'Yes');end; end
...
if any(options==9),
    ...
    validsubjects=1:CONN_x.Setup.nsubjects; %if isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'subjects'), validsubjects=CONN_x.gui.subjects; else validsubjects=1:CONN_x.Setup.nsubjects; end
    if isfield(CONN_x,'pobj')&&isstruct(CONN_x.pobj)&&isfield(CONN_x.pobj,'subjects'), validsubjects=CONN_x.pobj.subjects; conn_projectmanager('addstep',9,analyses); end
	h=conn_waitbar(0,['Step ',num2str(sum(options<=9)),'/',num2str(length(options)),': Updating Analysis variables']);
    nconditions=length(CONN_x.Setup.conditions.names)-1;
    if isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'conditions'), validconditions=CONN_x.gui.conditions; else validconditions=1:length(CONN_x.Setup.conditions.names)-1; end
    icondition=[];isnewcondition=[];for ncondition=validconditions,[icondition(ncondition),isnewcondition(ncondition)]=conn_conditionnames(CONN_x.Setup.conditions.names{ncondition}); end
    if any(isnewcondition(validconditions)), error(['Some conditions have not been processed yet. Re-run previous step']); end
    if any(CONN_x.Setup.steps([1,2,4])) && ~(isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'steps')&&~any(CONN_x.gui.steps([1,2,4])))
        [path,name,ext]=fileparts(CONN_x.filename);
        filepath=CONN_x.folders.preprocessing;
        ...
        for ianalysis=analyses,
            ...
            else, initial=CONN_x.Analyses(ianalysis).variables.names; dims={}; ders={}; fbands={}; end
        ...
        CONN_x.dynAnalyses.variables.names={};
        for n1=1:length(x1.names), 
            idx=strmatch(x1.names{n1},CONN_x.Preproc.confounds.names,'exact'); 
            %if isempty(idx),
            %    idx=strmatch(x1.names{n1},CONN_x.Preproc.confounds.names);  % allows partial-name matches
            ...
            %end
            if isempty(idx),
                CONN_x.dynAnalyses.variables.names{end+1}=x1.names{n1};
            end
        end
        if isfield(CONN_x.dynAnalyses.regressors,'names') && ~isempty(CONN_x.dynAnalyses.regressors.names), initial=CONN_x.dynAnalyses.regressors.names;
        else, initial=CONN_x.dynAnalyses.variables.names; end
        CONN_x.dynAnalyses.regressors.names={};
        for n1=1:length(initial),
            idx=strmatch(initial{n1},CONN_x.dynAnalyses.variables.names,'exact');
            if isempty(idx),
                idx=strmatch(initial{n1},CONN_x.dynAnalyses.variables.names); % allows partial-name matches
                if numel(idx)~=1, idx=[]; end
            end
            if ~isempty(idx)&&~strcmp(initial{n1},'Grey Matter')&&~strcmp(initial{n1},'White Matter')&&~strcmp(initial{n1},'CSF'),
                CONN_x.dynAnalyses.regressors.names{end+1}=CONN_x.dynAnalyses.variables.names{idx};
            end
        end
        
    end
    if any(CONN_x.Setup.steps([3]))&&(nargin<=1||any(varargin{1}==0)) && ~(isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'steps')&&~any(CONN_x.gui.steps([3]))),
        ...
        if isfield(CONN_x.vvAnalyses.regressors,'names') && ~isempty(CONN_x.vvAnalyses.regressors.names), 
            ...
            initial=CONN_x.vvAnalyses.variables;
            idx=find(strcmp(initial.names,'connectome-MVPA'),1);
            if ~isempty(idx)
        ...
        for n1=1:numel(initial.names)
            ...
            if isempty(idx),
                idx=strmatch(initial.names{n1},CONN_x.vvAnalyses.variables.names); % allows partial-name matches
                if numel(idx)~=1, idx=[]; end
            end
            optionsnames=fieldnames(CONN_x.vvAnalyses.variables);
            for n2=1:numel(optionsnames),
                if ~isempty(idx)&&(~isfield(initial,optionsnames{n2})||numel(initial.(optionsnames{n2}))<n1||isempty(initial.(optionsnames{n2}){n1})), CONN_x.vvAnalyses.regressors.(optionsnames{n2}){n1}=CONN_x.vvAnalyses.variables.(optionsnames{n2}){idx(1)};
...
%                         end
                        ...
                        elseif ~isempty(regexp(CONN_x.Analyses(ianalysis).modulation,'^Dynamic factor \d+$')), % Dyn FC
                            filename=fullfile(CONN_x.folders.preprocessing,['dyn_Subject',num2str(nsub,'%03d'),'.mat']);
                            xmod=load(filename);
...
%         ianalysis=CONN_x.Analysis;
        filepathresults=fullfile(CONN_x.folders.firstlevel,CONN_x.Analyses(ianalysis).name);
        REDO='Yes';%filename=fullfile(filepathresults,['resultsROI_Subject',num2str(1,'%03d'),'_Condition',num2str(icondition(validconditions(1)),'%03d'),'.mat']);
        %if ~isempty(dir(filename)),if isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'overwrite'), REDO=CONN_x.gui.overwrite; else, REDO=questdlg('Overwrite existing subject results?','','Yes', 'No', 'Yes');end; end
...
%                     xyz=cat(2,{xyz2t{idxroi1roi2}},{xyz2{setdiff(1:nrois2,idxroi1roi2)}});
                    ...
                    if nanalyses==1&&nsub==validsubjects(1) && ncondition==validconditions(1),N=numel(validsubjects)*numel(validconditions)*nrois2*length(analyses); nrois2bak=nrois2;
                    elseif nsub==validsubjects(1) && ncondition==validconditions(1), N=N-numel(validsubjects)*numel(validconditions)*nrois2bak+numel(validsubjects)*numel(validconditions)*nrois2; end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if any(options==13|options==13.5) && any(CONN_x.Setup.steps([3])) && (nargin<=1||any(varargin{1}==0)) && ~(isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'steps')&&~any(CONN_x.gui.steps([3]))),
    warning('off','MATLAB:DELETE:FileNotFound');
    [path,name,ext]=fileparts(CONN_x.filename);
    filepath=CONN_x.folders.preprocessing;
    ...
    if any(missingdata), disp(['Some subjects have not been processed yet. Re-run previous step']); return; end
    ...
    idx=find(cellfun(@(a,b)(isequal(a,1)|isequal(a,2))&isequal(b,3),CONN_x.vvAnalyses.regressors.deriv,CONN_x.vvAnalyses.regressors.dimensions_out));
    if ~isempty(idx), CONN_x.vvAnalyses.regressors.dimensions_out(idx)=repmat({2+~issurface},1,numel(idx)); end
    ...
    if nmeasures2>0 && ~isequal(validsubjects,1:CONN_x.Setup.nsubjects), disp('MVPA analysis parallelization not yet available (run locally or as a single job instead). Skipping these analyses'); end
    if nmeasures2>0 && isequal(validsubjects,1:CONN_x.Setup.nsubjects),% && ~CONN_x.pobj.isextended, % MVPA
        isnew=isnew0;
        MAXDIMS=256;
%         h=conn_waitbar(0,['Step ',num2str(sum(options<=13)),'/',num2str(length(options)),': connectome-level MVPA analyses']);
        ...
        if strcmp(lower(REDO),'yes')||any(isnew(nmeasures1+(1:nmeasures2))),
            ...
            %                 sA=prod(Y1(1).dim);
            %                 [gridx,gridy,gridz]=ndgrid(1:Y1(1).dim(1),1:Y1(1).dim(2),1:Y1(1).dim(3));xyz=[gridx(:),gridy(:),gridz(:),ones(numel(gridx),1)]';
            filename_B1=fullfile(filepath,['vvPC_Subject',num2str(1,'%03d'),'_Condition',num2str(icondition(validconditions(1)),'%03d'),'.mat']);
            Y1=conn_vol(filename_B1);
                ...
                if strcmp(lower(REDO),'yes')||isnew(nmeasure)
                    ...
                    for nsub=1:CONN_x.Setup.nsubjects
                        for ncondition=validconditions,
                            ...
                            if isfield(Yout,'EmptyData')&&Yout.EmptyData, Y1MD(nsub,ncondition)=1; end
                        end
                    end
                    ...
                    for slice=1:Y1.size.Ns,
                        for nsub=1:CONN_x.Setup.nsubjects
                            for ivalidcondition=1:numel(validconditions),
                                ...
                                if nsub==1&&ncondition==validconditions(1), x=zeros([size(temp,2),NdimsIn,CONN_x.Setup.nsubjects,numel(validconditions)]); end
                                if ~isempty(temp)
                                    x(:,1:min(size(temp,1),NdimsIn),nsub,ivalidcondition)=temp(1:min(size(temp,1),NdimsIn),:)';
                                end
                            end
                    ...
                    save(filename,'C');
                    C=permute(reshape(C,[NdimsIn,CONN_x.Setup.nsubjects,numel(validconditions),NdimsIn,CONN_x.Setup.nsubjects,numel(validconditions)]),[1,4,2,3,5,6]);
                    NdimsOut=min(CONN_x.Setup.nsubjects*numel(validconditions),measures.dimensions_out{nmeasure});
                    ...
                    for nsub=1:CONN_x.Setup.nsubjects
                        ...
                        for ncondition=validconditions,
                            for ndim=1:NdimsOut
                                ...
                                if Y1MD(nsub,ncondition), temp.descrip='CONNlabel:MissingData'; end
...
%                     Dout=conn_init_vol(Dout);
                    ...
                    for slice=1:Y1.size.Ns,
                        ...
                        c=zeros([Y1.size.Nv(slice),repmat([CONN_x.Setup.nsubjects,numel(validconditions)],[1,2])]);
                        for nsub=1:CONN_x.Setup.nsubjects
                            for ivalidcondition=1:numel(validconditions)
                                ncondition=validconditions(ivalidcondition);
                                for nsub2=1:CONN_x.Setup.nsubjects
                                    for ivalidcondition2=1:numel(validconditions)
                                        ...
                                        c(:,nsub,ivalidcondition,nsub2,ivalidcondition2)=sum((x(:,:,nsub,ivalidcondition)*C(:,:,nsub,ivalidcondition,nsub2,ivalidcondition2)).*x(:,:,nsub2,ivalidcondition2),2);
                                    end
                                end
                            end
                        end
                        x=zeros([CONN_x.Setup.nsubjects*numel(validconditions),NdimsOut,Y1.size.Nv(slice)]); 
                        dcov=zeros([NdimsOut,Y1.size.Nv(slice)]);
                        %nvox0=sum(Y1.size.Nv(1:slice-1));
                        for nvox=1:Y1.size.Nv(slice),
                            c1=reshape(c(nvox,:),CONN_x.Setup.nsubjects*numel(validconditions)*[1,1]);
                        ...
                        for nsub=1:CONN_x.Setup.nsubjects
                            for ivalidcondition=1:numel(validconditions)
                                ...
                                for ndim=1:NdimsOut
                                    ...
                                    t(idx)=squeeze(x(sub2ind([CONN_x.Setup.nsubjects,numel(validconditions)],nsub,ivalidcondition),ndim,:));
...
%                         answ=questdlg('Data has been processed using an old CONN version. Need to re-run Denoising.ROI step to be able to continue. This step should take only a few minutes. Do this now?','','Yes','No','Yes'); 
%                         if isequal(answ,'Yes')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if any(options==14) && any(CONN_x.Setup.steps([3])) && ~(isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'steps')&&~any(CONN_x.gui.steps([3]))),
    warning('off','MATLAB:DELETE:FileNotFound');
    [path,name,ext]=fileparts(CONN_x.filename);
    filepath=CONN_x.folders.preprocessing;
    filepathresults=CONN_x.folders.firstlevel;
...
%             foldername=txt{1};
    ...
    if isempty(REDO)&&~all(missingdata),
        if isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'overwrite'), REDO=CONN_x.gui.overwrite; else, REDO=questdlg('Overwrite existing results?','','Yes', 'No', 'Yes');end;
    else REDO='Yes'; 
    end
...
%     if nargin>1, dosinglecontrast=(varargin{1}==1); else, dosinglecontrast=length(nsources)~=1; end; %~(length(nsources)>1 & all(csources==1/length(nsources))); end
    if isfield(CONN_x.Results,'foldername')&&~isempty(CONN_x.Results.foldername),
        ...
        if isdir(fullfile(filepathresults2,foldername_back))
            foldername=foldername_back; % backwards-compatibility with existing results
        end
...
%     REDUCENAMES=(state==2);
    if dosinglecontrast>0,
        ...
        if dosinglecontrast==2,
            ...
            elseif ~(isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'overwrite')), REDO=questdlg('Re-estimate/Overwrite existing second-level results?','','Yes', 'No', 'No'); if strcmp(lower(REDO),'yes'),dosinglecontrast=1; end; end
        end
        if dosinglecontrast==1,
            ...
            for nsub=1:length(nsubjects),%CONN_x.Setup.nsubjects,
                ...
                if numel(nconditions)>1&&size(cconditions,1)==1
                    for n1=1:numel(nsources)
                        ...
                        contrastname{n1,numel(nconditions)+1}=tcontrastname;
                    end
                    filename=filename(:,end);
                    contrastname=contrastname(:,end);
                    new_cconditions=1;
                else
                ...
                if numel(nsources)>1&&size(csources,1)==1
                    for n0=1:size(filename,2)
                        ...
                        contrastname{numel(nsources)+1,n0}=tcontrastname;
                    end
                    filename=filename(end,:);
                    contrastname=contrastname(end,:);
                    new_csources=1;
            ...
            SPMall(1).xX.isSurface=isequal(SPMall(1).xY.VY(1).dim,conn_surf_dims(8).*[1 1 2]);
...
%                         end
        ...
        for n1=1:length(sources),
            ...
            SPMall(n1).xX.isSurface=isequal(SPMall(n1).xY.VY(1).dim,conn_surf_dims(8).*[1 1 2]);
            ...
            case 1, % random-effects
                for n1=1:length(SPMall),
                    ...
                    if isfield(SPM.xX,'isSurface')&&SPM.xX.isSurface
                        %SPM.xX_multivariate.C=csubjecteffects;
                        %SPM.xX_multivariate.M=contrast;
                        save('SPM.mat','SPM');
                        spm_unlink('mask.img');
                        files=dir('spmT_*.cluster.mat');
                        if ~isempty(files)
                            files={files.name};
                            spm_unlink(files{:});
                        end
                        ...
                        spm_write_vol(V,double(~any(isnan(Y)|Y==0,4)));
                        Y=permute(reshape(Y,[SPM.xY.VY(1).dim(1:3) size(SPM.xY.VY)]),[4,5,1,2,3]);
                        for n2=1:size(Y,5)
                            if n2==1
                                [SPM.xX_multivariate.h,SPM.xX_multivariate.F,nill,SPM.xX_multivariate.dof,SPM.xX_multivariate.statsname]=conn_glm(SPM.xX_multivariate.X,Y(:,:,:,:,n2),SPM.xX_multivariate.C,SPM.xX_multivariate.M);
                            else
                                [SPM.xX_multivariate.h(:,:,:,:,n2),SPM.xX_multivariate.F(:,:,:,:,n2)]=conn_glm(SPM.xX_multivariate.X,Y(:,:,:,:,n2),SPM.xX_multivariate.C,SPM.xX_multivariate.M);
                            end
                            conn_waitbar(1/2+1/2*(n2/size(Y,5)/(length(SPMall))),h,sprintf('Analysis %d',n1));
                        end
                        save('SPM.mat','SPM');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if any(options==20) && any(CONN_x.Setup.steps([4])) && ~(isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'steps')&&numel(CONN_x.gui.steps)>3&&~any(CONN_x.gui.steps([4]))),
    [path,name,ext]=fileparts(CONN_x.filename);
    filepath=CONN_x.folders.preprocessing;
    filepathresults=CONN_x.folders.preprocessing;
	nconditions=length(CONN_x.Setup.conditions.names)-1;
    if isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'conditions'), validconditions=CONN_x.gui.conditions; else validconditions=1:length(CONN_x.Setup.conditions.names)-1; end
    icondition=[];isnewcondition=[];for ncondition=validconditions,[icondition(ncondition),isnewcondition(ncondition)]=conn_conditionnames(CONN_x.Setup.conditions.names{ncondition},'+'); end
    %h=conn_waitbar(0,['Step ',num2str(sum(options<=20)),'/',num2str(length(options)),': dynamic temporal-components estimation']);
    if any(arrayfun(@(n)isempty(dir(fullfile(filepath,['ROI_Subject',num2str(n,'%03d'),'_Condition',num2str(0,'%03d'),'.mat']))),1:CONN_x.Setup.nsubjects)), error(['Data not ready yet. Re-run Denoising step']); end
    REDO='Yes'; filename=fullfile(filepathresults,['dyn_Subject',num2str(1,'%03d'),'.mat']);
    if ~isempty(dir(filename)),if isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'overwrite'), REDO=CONN_x.gui.overwrite; else, REDO=questdlg('Overwrite existing subject results?','','Yes', 'No', 'Yes');end; end
    Ncomponents=CONN_x.dynAnalyses.Ncomponents; % number of components to estimate
    selectedcondition=CONN_x.dynAnalyses.condition;    % selected condition (determines span of BOLD timeseries to use in these analyses)
    names=CONN_x.dynAnalyses.regressors.names;  % cell-array of ROI names (empty to use all ROIs)
    filter=CONN_x.dynAnalyses.filter;
    if strcmp(lower(REDO),'yes')||isempty(dir(filename))
        REDO='Yes';
        X=[];
        Y=[];
        ...
        for nsub=1:CONN_x.Setup.nsubjects,
            ...
            if isempty(dir(filename)), disp(['Not ready to process step conn_process_20']); return; end %conn_waitbar('close',h);return; end
            X1=load(filename);
            if isempty(names), names=X1.names(~ismember(X1.names,{'White Matter','CSF'})); end
            [ok,idx]=ismember(names,X1.names); 
            if ~all(ok), error('Missing ROI data in subject %d',nsub); end
            y=cat(2,X1.data{idx});
            if nsub>1&&size(y,2)~=size(Y,2), error('Incorrect number of ROI timeseries in subject %d',nsub); end
            ...
            if ~isempty(filter)&&filter<1/tr/2, 
                nsess=max(X1.data_sessions);
                xfilter=[];
                for nses=1:nsess
                    nt=CONN_x.Setup.nscans{nsub}{nses};
                    nh=min(nt,1+2*floor(1/filter/tr));
            ...
            if nsub==1, COND_names=X1.conditionsnames(ok); COND_weights=cell(1,nnz(ok)); end
        ...
        drawnow;
        [H,B,H0,B0]=conn_invPPI(Y,Ncomponents,[],Xfilter,1);
        CONN_x.dynAnalyses.sources=CONN_x.dynAnalyses.regressors.names;
        filename=fullfile(filepathresults,'dyn_Base.mat');
        names=arrayfun(@(n)sprintf('Dynamic factor %02d',n),1:Ncomponents,'uni',0);
        ROInames=CONN_x.dynAnalyses.sources;
        selectedconditionname=CONN_x.Setup.conditions.names{selectedcondition};
        ...
        filename=fullfile(filepathresults,'dyn_Base.mat');
        load(filename,'X','Y','Xfilter','B','H','B0','H0','names','ROInames','IDX_subject','IDX_session','COND_names','COND_weights','selectedconditionname');
    ...
    if CONN_x.dynAnalyses.output(2)
        new_names={};
        new_values={};
        for ncomp=1:Ncomponents
            for n=1:numel(COND_names)
                ...
                H_avg_weighted=accumarray(IDX_subject,H(:,ncomp).*max(0,COND_weights{n}),[CONN_x.Setup.nsubjects,1])./w;
                if strcmp(CONN_x.Setup.conditions.names{selectedcondition},'rest'), new_names{end+1}=sprintf('Dynamic baseline factor %02d @ %s',ncomp,COND_names{n});
        ...
        for ncomp=1:Ncomponents
            ...
            for n=1:numel(COND_names)
                w=max(eps,accumarray(IDX_subject,max(0,COND_weights{n}),[CONN_x.Setup.nsubjects,1]));
                H_std_weighted=sqrt(max(0,accumarray(IDX_subject,H(:,ncomp).^2.*max(0,COND_weights{n}),[CONN_x.Setup.nsubjects,1])./w - (accumarray(IDX_subject,H(:,ncomp).*max(0,COND_weights{n}),[CONN_x.Setup.nsubjects,1])./w).^2));
                if strcmp(CONN_x.Setup.conditions.names{selectedcondition},'rest'), new_names{end+1}=sprintf('Dynamic modulation factor %02d @ %s',ncomp,COND_names{n});
                else new_names{end+1}=sprintf('Dynamic modulation factor %s_%02d @ %s',CONN_x.Setup.conditions.names{selectedcondition},ncomp,COND_names{n});
                end
        ...
        for n=1:numel(COND_names)
            ...
            H_std_weighted=0;
            for ncomp=1:Ncomponents
                H_std_weighted=H_std_weighted+(max(0,accumarray(IDX_subject,H(:,ncomp).^2.*max(0,COND_weights{n}),[CONN_x.Setup.nsubjects,1])./w - (accumarray(IDX_subject,H(:,ncomp).*max(0,COND_weights{n}),[CONN_x.Setup.nsubjects,1])./w).^2));
            end
            if strcmp(CONN_x.Setup.conditions.names{selectedcondition},'rest'), new_names{end+1}=sprintf('Dynamic modulation total @ %s',COND_names{n});
            else new_names{end+1}=sprintf('Dynamic modulation total_%s @ %s',CONN_x.Setup.conditions.names{selectedcondition},COND_names{n});
            end
            new_values{end+1}=sqrt(H_std_weighted);
        end
        for ncomp=1:Ncomponents
            for n=1:numel(COND_names)
                tempH=abs(convn(H(max(1,min(size(H,1),0:size(H,1)+1)),ncomp),[-1;0;1],'valid'));
                tempW=max(0,COND_weights{n});
                maskout=IDX_subject~=[nan;IDX_subject(1:end-1)]|IDX_subject~=[IDX_subject(2:end);nan]|IDX_session~=[nan;IDX_session(1:end-1)]|IDX_session~=[IDX_session(2:end);nan];
                tempH(maskout)=0;
                tempW(maskout)=0;
                w=max(eps,accumarray(IDX_subject,tempW,[CONN_x.Setup.nsubjects,1]));
                H_avg_weighted=accumarray(IDX_subject,tempH.*tempW,[CONN_x.Setup.nsubjects,1])./w;
                H_std_weighted=max(0,accumarray(IDX_subject,H(:,ncomp).^2.*tempW,[CONN_x.Setup.nsubjects,1])./w - (accumarray(IDX_subject,H(:,ncomp).*tempW,[CONN_x.Setup.nsubjects,1])./w).^2);
                if strcmp(CONN_x.Setup.conditions.names{selectedcondition},'rest'), new_names{end+1}=sprintf('Dynamic rate of change factor %02d @ %s',ncomp,COND_names{n});
                else new_names{end+1}=sprintf('Dynamic rate of change factor %s_%02d @ %s',CONN_x.Setup.conditions.names{selectedcondition},ncomp,COND_names{n});
                end
                new_values{end+1}=H_avg_weighted./sqrt(max(eps,H_std_weighted));
            end
        end
        for n=1:numel(new_names)
            icov=strmatch(new_names{n},CONN_x.Setup.l2covariates.names,'exact');
            if isempty(icov), icov=numel(CONN_x.Setup.l2covariates.names); CONN_x.Setup.l2covariates.names{end+1}=' '; end
            CONN_x.Setup.l2covariates.names{icov}=new_names{n};
            for n1=1:CONN_x.Setup.nsubjects, CONN_x.Setup.l2covariates.values{n1}{icov}=new_values{n}(n1); end
        end
    end
    if CONN_x.dynAnalyses.output(3)
        for ncomp=1:Ncomponents
            if strcmp(CONN_x.Setup.conditions.names{selectedcondition},'rest'), name=sprintf('Dynamic factor %02d',ncomp);
    ...
    if strcmp(lower(REDO),'yes')
        if CONN_x.dynAnalyses.output(1)
            newanalyses=[];
            for ncomp=1:Ncomponents
                if strcmp(CONN_x.Setup.conditions.names{selectedcondition},'rest'), name=sprintf('Dynamic factor %02d',ncomp);
                else name=sprintf('Dynamic factor %s_%02d',CONN_x.Setup.conditions.names{selectedcondition},ncomp);
                ...
                ianalysis=find(strcmp(names,name));
                if isempty(ianalysis), ianalysis=numel(CONN_x.Analyses)+1; end
                newanalyses=[newanalyses ianalysis];
                CONN_x.Analyses(ianalysis).name=name;
                CONN_x.Analyses(ianalysis).modulation=sprintf('Dynamic factor %02d',ncomp);
                CONN_x.Analyses(ianalysis).measure=1;
                CONN_x.Analyses(ianalysis).weight=2;
                CONN_x.Analyses(ianalysis).type=1;
                CONN_x.Analyses(ianalysis).regressors.names=CONN_x.dynAnalyses.regressors.names;
                CONN_x.Analyses(ianalysis).regressors.dimensions=repmat({1},size(CONN_x.dynAnalyses.regressors.names));
                CONN_x.Analyses(ianalysis).regressors.deriv=repmat({0},size(CONN_x.dynAnalyses.regressors.names));
                CONN_x.Analyses(ianalysis).regressors.types=repmat({'roi'},size(CONN_x.dynAnalyses.regressors.names));
                CONN_x.Analyses(ianalysis).regressors.fbands=repmat({1},size(CONN_x.dynAnalyses.regressors.names));
                CONN_x.Analyses(ianalysis).variables=CONN_x.Analyses(ianalysis).regressors;
                %CONN_x.Analyses(ianalysis).variables=CONN_x.Analyses(1).variables;
...
%         if ~isempty(regexp(varargin{2},'^Step \d+/\d+:\s*'))
%             b1=regexp(varargin{2},'^Step (\d+)/(\d+):\s*','tokens','once');
%             steps=str2double(b1);
...
%         if numel(steps)==2, str=[str, ' [',num2str((steps(1)-1+varargin{1})/steps(2),'%0.3f'),']']; end
        conn_cumdisp(str); 
    end
end
end
