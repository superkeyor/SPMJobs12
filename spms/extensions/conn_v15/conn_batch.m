...
%   surfacesmoothing: Smoothing level for surface-based analyses (number of discrete diffusion steps) [10]
% 
...
%   unwarp_functionals: (for Setup.preprocessing.steps=='realign&unwarp&phasemap') unwarp_functionals{nsub}{nses} char array of Phase Map volume (vdm* file) 
%   cwthreshold     : cwthreshold(1): CSF&WhiteMatter threshold for noise aCompCor region; cwthreshold(2): CSF&WhiteMatter number of mask erosion steps ([.5 1])
...
%                     When using add=1, the following fields are expected to contain the information for the new/added subjects only: Setup.functionals, Setup.structurals, Setup.roiextract_functionals, Setup.unwarp_functionals, Setup.spmfiles, Setup.masks.Grey/White/CSF, Setup.rois.files, Setup.conditions.onsets/durations, Setup.covariates.files, Setup.preprocessing.unwarp
...
%                     When using Setup.add=1 the bath.subjects field is disregarded/overwritten to point to the new/added subjects only
...
%                     steps={'default_mni','default_mniphase','default_ss','default_ssphase',...    % (default pipelines: default_mni (for volume-based analyses, MNI space); default_mniphase (for volume-based analyses, MNI space, phase-maps are available); default_ss (for surface-based analyses, subject-space); default_ssphase (for surface-based analyses, subject-space, phase-maps are available)  
...
%     subjects        : list of subjects to apply preprocessing steps (defaults to all subjects)
...
%     unwarp          : (functional_realign&unwarp) unwarp{nsub}{nses} char array of phase maps (*vdm file) 
...
if isfield(batch,'Setup'),
    ...
    if isfield(batch.Setup,'spmfiles')&&~isempty(batch.Setup.spmfiles),
        ...
        conn_importspm(batch.Setup.spmfiles,'subjects',SUBJECTS);
        CONN_x.gui=1;
    end
    ...
    if isfield(batch.Setup,'roiextract_functionals')
        for isub=1:numel(SUBJECTS),
            ...
            for nses=1:CONN_x.Setup.nsessions(nsub),
                [CONN_x.Setup.roiextract_functional{nsub}{nses},V]=conn_file(batch.Setup.roiextract_functionals{isub}{nses});
                %CONN_x.Setup.nscans{nsub}{nses}=prod(size(V));
            end
        end
    end
...
if isfield(batch,'parallel')&&isfield(batch.parallel,'N')&&batch.parallel.N>0,
    ...
    if isequal(SUBJECTS,1:CONN_x.Setup.nsubjects), Njobs=batch.parallel.N;
    else
        Ns=numel(SUBJECTS);
        N=min(Ns,parallel_N);
        Njobs=arrayfun(@(n)SUBJECTS(floor(Ns/N*(n-1))+1:min(Ns,floor(Ns/N*n))),1:N,'uni',0);
    end
    info=conn_jobmanager('submit',PAR_CMD,Njobs,PAR_ARG);
