function data=conn_randomise(X,Y,c1,c2,Pthr,Pthr_type,Pthr_side,niters,filename,overwrite,datatype,analysismask)
...
if nargin<11||isempty(datatype), datatype='matrix'; end % 'matrix'|'surface'
if nargin<12, analysismask=[]; end
maxT=100;
[Nn,Nv,Nr1,Nr2]=size(Y);

% remove orthogonal contrasts
nc=null(c1);
xnc=X*nc;
ix=pinv(xnc'*xnc);
r=zeros(Nn,size(c2,1),Nr1*Nr2);
for nr=1:Nr1*Nr2
    r(:,:,nr)=Y(:,:,nr)*c2'-xnc*(ix*(xnc'*Y(:,:,nr)*c2'));        % removes from y
end
...
Hist_Cluster_size=[Hist_Cluster_size,repmat({sparse(1+Nr1*Nr2,1)},1,nthr)];
Hist_Cluster_mass=[Hist_Cluster_mass,repmat({sparse(1+maxT*Nr1*Nr2,1)},1,nthr)];
...
Hist_Seed_size=[Hist_Seed_size,repmat({sparse(1+Nr1*Nr2,1)},1,nthr)];
Hist_Seed_mass=[Hist_Seed_mass,repmat({sparse(1+maxT*Nr1*Nr2,1)},1,nthr)];
...
if univariate
    ix=pinv(x'*x);
    Cyy=reshape(sum(abs(r).^2,1),[1,Nr1*Nr2]);
    k_dof=sqrt(dof*x'*x);
    r=r(:,:);
else
...
if isdisplay, ht=conn_timedwaitbar(0,'Updating permutation statistics. Please wait'); 
else disp('Updating permutation statistics. Please wait'); 
end
...
for niter=1:niters
    ...
    for i=thridx % all new threshold values
        if     univariate
            if Pthr_type(i)==1
                if Pthr_side(i)==1,     show=f(:)>=Xthr(i);
                elseif Pthr_side(i)==2, show=f(:)<=Xthrb(i);
                elseif Pthr_side(i)==3, show=abs(f(:))>=Xthrc(i);
                else error('incorrect option Pthr_side');
                end
            elseif Pthr_type(i)==2
                if Pthr_side(i)==1,     show=p2(:)<=Pthr(i);
                elseif Pthr_side(i)==2, show=pb2(:)<=Pthr(i);
                elseif Pthr_side(i)==3, show=pc2(:)<=Pthr(i);
                else error('incorrect option Pthr_side');
                end
            elseif Pthr_type(i)==3
                if Pthr_side(i)==1,     show=p3(:)<=Pthr(i);
                elseif Pthr_side(i)==2, show=pb3(:)<=Pthr(i);
                elseif Pthr_side(i)==3, show=pc3(:)<=Pthr(i);
                else error('incorrect option Pthr_side');
                end
            elseif Pthr_type(i)==4
                if Pthr_side(i)==1,     show=f(:)>=Xthr(i);
                elseif Pthr_side(i)==2, show=f(:)<=-Xthr(i);
                elseif Pthr_side(i)==3, show=abs(f(:))>=Xthr(i);
                else error('incorrect option Pthr_side');
                end
            else error('incorrect option Pthr_type');
            end
        elseif Pthr_type(i)==1, show=p(:)<=Pthr(i);
        elseif Pthr_type(i)==2, show=p2(:)<=Pthr(i);
        elseif Pthr_type(i)==3, show=p3(:)<=Pthr(i);
        elseif Pthr_type(i)==4, show=f(:)>=Xthr(i);
        else error('incorrect option Pthr_type');
        end
        
        % cluster-size & cluster-mass for each cluster
        [nclL,CLUSTER_labels]=conn_clusters(reshape(show,[Nr1,Nr2]),datatype);
        ...
        Hist_Cluster_size{i}=Hist_Cluster_size{i}+sparse(1+nclL,1,1/numel(nclL),Nr1*Nr2+1,1);   % histogram of cluster size (note: Hist_(1) corresponds to cluster size=0)
        ...
        Hist_Cluster_mass{i}=Hist_Cluster_mass{i}+sparse(1+round(min(maxT*Nr1*Nr2,maxT*mclL)),1,1/numel(mclL),maxT*Nr1*Nr2+1,1);   % histogram of cluster mass (note: Hist_(1) corresponds to cluster mass=0)
        ...
        if strcmp(datatype,'matrix')
            ...
            Hist_Seed_size{i}=Hist_Seed_size{i}+sparse(1+nsdL,1,1/numel(nsdL),Nr1*Nr2+1,1);   % histogram of seed size (note: Hist_(1) corresponds to seed size=0)
            ...
            Hist_Seed_mass{i}=Hist_Seed_mass{i}+sparse(1+round(min(maxT*Nr1*Nr2,maxT*msdL)),1,1/numel(msdL),maxT*Nr1*Nr2+1,1);   % histogram of seed mass (note: Hist_(1) corresponds to seed mass=0)
        end
    end
    
    if ~rem(niter,1),
        if isdisplay, conn_timedwaitbar(niter/niters,ht); 
        else fprintf('.'); 
        end
    end
end
...
if isdisplay, close(ht); 
else fprintf('\n');
end
rand('state',randstate);
if ~nargout
    save(filename,'Pthr','Pthr_type','Pthr_side','maxT','Hist_Cluster_size','Hist_Cluster_mass','Dist_Cluster_sizemax','Dist_Cluster_massmax','Hist_Seed_size','Hist_Seed_mass','Dist_Seed_sizemax','Dist_Seed_massmax');
else
    data=struct('Pthr',Pthr,'Pthr_type',Pthr_type,'Pthr_side',Pthr_side,'maxT',maxT,'Hist_Cluster_size',{Hist_Cluster_size},'Hist_Cluster_mass',{Hist_Cluster_mass},'Dist_Cluster_sizemax',{Dist_Cluster_sizemax},'Dist_Cluster_massmax',{Dist_Cluster_massmax},'Hist_Seed_size',{Hist_Seed_size},'Hist_Seed_mass',{Hist_Seed_mass},'Dist_Seed_sizemax',{Dist_Seed_sizemax},'Dist_Seed_massmax',{Dist_Seed_massmax});
end
