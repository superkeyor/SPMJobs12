...
% labels connected components (bwlabel for arbitrary connectivity)
...
switch(datatype)
    case 'matrix' % uses default node-connectivity for ROI-to-ROI analyses
        ...
        M=M|M';
        %[p,q,r,s]=dmperm(M|speye(size(M,1)));
        valid=find(any(M,1));
    ...
    otherwise, % uses arbitrary node-connectivity
        if isempty(A)
        ...
        if isstruct(A)||size(A,2)==3, A=spm_mesh_adjacency(A); end
        [n,n2]=size(mask);
        mask=mask>0;
