...
state.xyz_x=reshape(xyz(1,:),state.size);
state.xyz_y=reshape(xyz(2,:),state.size);
state.xyz_z=reshape(xyz(3,:),state.size);
state.xyz_range=[min(state.xyz_x(:)) max(state.xyz_y(:)); min(state.xyz_y(:)) max(state.xyz_z(:)); min(state.xyz_x(:)) max(state.xyz_z(:))];
if isempty(state.structural), state.structural=fullfile(fileparts(which('spm')),'canonical','single_subj_T1.nii'); end
%filename=fullfile(fileparts(which('spm')),'canonical','avg152T1.nii');
if ischar(state.structural)
    V=spm_vol(state.structural);
    V=V(1);
    state.structural=reshape(spm_get_data(V,pinv(V.mat)*xyz),state.size);
end
if state.isvol
...
state.handles.mode=uicontrol('style','togglebutton','units','norm','position',[0 0 .5 .05],'string','Click on image to select reference point','value',1,'callback',{@conn_slice_display_refresh,'togglepointer'});
...
state.handles.slider=uicontrol('style','slider','units','norm','position',[.47 .1 .025 .8],'callback',{@conn_slice_display_refresh,'pointer_vox','x'});
                ...
                if state.view(nview)
                    switch nview
                        case 1,
                            ...
                            z1=permute(state.structural(max(1,min(state.size(1),state.pointer_vox(1))),:,:),[2 3 1]);
                        ...
                        case 2,
                            ...
                            z1=permute(state.structural(:,max(1,min(state.size(2),state.pointer_vox(2))),:),[1 3 2]);
                        ...
                        case 3,
                            ...
                            z1=permute(state.structural(:,:,max(1,min(state.size(3),state.pointer_vox(3)))),[1 2 3]);
