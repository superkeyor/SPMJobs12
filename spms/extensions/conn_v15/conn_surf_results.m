...
if nargin<1||(isempty(filename)&&~ischar(filename)), 
    answ=questdlg('Surface statistics from:','select file type','SPM.mat','spmT_ file','SPM.mat');
...
for n1=1:numel(filenames)
    for n2=1:numel(hems)
        ...
        %[xyz,faces]=read_surf(fullfile(pathname,'utils','surf',[hems{n2},filenames{n1},'.surf'])); faces=faces+1;
        %PATCHES{n1,n2}=struct('vertices',xyz,'faces',faces);
    end
end
...
CROSSHAIR={[-100,100,nan,0,0,nan,0,0,nan],[0,0,nan,-100,100,nan,0,0,nan],[0,0,nan,0,0,nan,-100,100,nan]};

hfig=figure('numbertitle','off','menubar','none','units','norm','position',[.05 .3 .95 .55]);%,'closerequestfcn',@conn_surf_results_closerequestfcn);
...
%         uicontrol('style','pushbutton','units','norm','position',[0,.95,.1,.05],'string','Print','callback',@conn_surf_results_doprint,'tooltipstring','Print high-resolution images to file');
        ...
        hold on;
        hcrosshair=plot3(CROSSHAIR{:},'g--','linewidth',2);
        hold off;
    ...
    function conn_surf_results_closerequestfcn(varargin)
        answ=questdlg('Do you want to:','conn_surf_results','Exit','Continue','Exit');
