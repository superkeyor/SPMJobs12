...
%         else, nsources=varargin{1}; data.source=0; end
        ...
        color1=[1,1,1];
        color2=[1,1,1];
        hfig=figure('units','norm','position',[.05,.2,.9,.7],'numbertitle','off','name','Network theory second-level results','color',color1,'colormap',gray,'menubar','none');
...
switch(data.display),
    case 'connectivity',
        ...
        for n1=1:N,
            if n1<=N&&z(n1)>0,
                txt2{end+1}=(sprintf('%-20s %6.2f  %6.2f  %4d  %8.6f  %8.6f',data.namesreduced{data.displaytheserois(n1)},data.h(data.displaymeasure,n1),data.F(data.displaymeasure,n1),data.dof(data.displaymeasure,n1,end),p(n1),P(n1))); 
        ...
        if ~data.disptype, set(hfig,'colormap',.5+.5*gray(128)); else set(hfig,'colormap',gray(128)); end
        ...
        uimenu(hc1,'Label','View sagittal (y-y)','callback',{@conn_displaynetwork,'view-sagittal'});
...
%             n1=data.displaytheserois(na1);
        ...
        if datadisplay3d
            c=mat2cell(cmap(round(1+48*(1+sign(K(idxtext)))/2),:),ones(numel(idxtext),1),3);
            for n1=1:numel(data.source),idxc=find(idxtext==data.source(n1));c(idxc,:)=repmat({[.25,.25,.25]},[numel(idxc),1]); end
            conn_mesh_display('','',[],...
                struct('sph_names',{data.namesreduced(data.displaytheserois(idxtext))},'sph_xyz',[data.x(data.displaytheserois(idxtext)),data.y(data.displaytheserois(idxtext)),data.z(data.displaytheserois(idxtext))],...
                'sph_r',max(1,6*K(idxtext)).*(K(idxtext)>0),... %6*ones(numel(idxtext),1),...
                'sph_c',{c}), ...
                (1*Zthrdisplay(idxtext,idxtext)).*(Zthrdisplay(idxtext,idxtext)>Zthrdisplay_show), ...
