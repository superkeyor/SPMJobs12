		...
		case {'cursormove','cursorup','cursorout'}
            ...
            if ~strcmp(handle,'cursorout')&&md>-.01, % move inside active area 
                ...
                if any(CONN_MM.INSIDE~=[n0,n1]),
                    ...
                    if ~iscell(CONN_MM.MENU{n0}.callback{n1}),
                        conn_menumanager(CONN_MM.MENU{n0}.callback{n1},'on',1);
                    end
                end
            ...
            if ~any(CONN_MM.INSIDE)&&~isempty(CONN_MM.onregionarea)
                ...
                if ~strcmp(handle,'cursormove'),
                    for n0=reshape(setdiff(1:numel(CONN_MM.onregioncallback),[md,CONN_MM.onregioninside]),1,[]),
                        if all(ishandle(CONN_MM.onregioncallback{n0}))
                            if CONN_MM.onregionvisible(n0)==0||xor(all(strcmp(get(CONN_MM.onregioncallback{n0},'visible'),'on')), CONN_MM.onregionvisible(n0)>0), %onregions=[onregions CONN_MM.onregionhandle{n0}];
                            else offregions=[offregions CONN_MM.onregionhandle{n0}];
                            end
                        end
                    end
                end
                ...
                for n0=md(:)' % mouse-movement inside region
                    if ~isempty(CONN_MM.onregionmotioncallback{n0}), 
...
%                     for n0=md,%setdiff(md,CONN_MM.onregioninside) %in
                ...
                if ~isempty(colorB), set(colorB,'foregroundcolor',[.3 .3 .3]+.4*(mean(CONN_gui.backgroundcolor)<.5)); end
                if ~isempty(colorC), set(colorC,'foregroundcolor',[0 0 0]+1*(mean(CONN_gui.backgroundcolor)<.5)); end
                if isempty(md), md=0; end
    ...
    if isempty(handle), 
        ...
        params=struct(...
            ...
            'color',[3/6,3/6,6/6; 2.5/6,2.5/6,6/6; 2/6,2/6,6/6],...     % 3x3 matrix of color components. Rows are status (cursor-out,cursor-in,selected) and columns are rgb components
            'horizontalalignment','center',...
            'transparent',.5,...
            'fontcolor',[.7,.7,.7; 1,1,1;1,1,1],...                             % 
            'fontname','Arial',...                              % 
            'fontsize',8,...									% 
            'fontweight','normal',...                             % 
            'fontangle','normal',...                             % 
        ...
        if strcmp(params.bordertype,'round'), 
            params.color=diag([1 1 1])*params.color(:,[3 2 1]); 
    ...
    for nhandle=1:length(handle),
        ...
        if CONN_MM.MENU{thishandle}.off || CONN_MM.MENU{thishandle}.on
            ...
            for n1=changed(:)',%1:length(CONN_MM.MENU{thishandle}.handle.axes),
				if numel(CONN_MM.MENU{thishandle}.handle.axes)>=n1&&ishandle(CONN_MM.MENU{thishandle}.handle.axes(n1)),
					delete(CONN_MM.MENU{thishandle}.handle.text{n1});
					%if ~CONN_gui.dounixGUIbugfix, delete(CONN_MM.MENU{thishandle}.handle.image(n1)); end
					delete(CONN_MM.MENU{thishandle}.handle.axes(n1)); 
				end
            end
        ...
        if CONN_MM.MENU{thishandle}.on,
            ...
            for n1=changed(:)',%1:CONN_MM.MENU{thishandle}.n,
				...
				if length(CONN_MM.MENU{thishandle}.handle.axes)>=n1 && ishandle(CONN_MM.MENU{thishandle}.handle.axes(n1)), delete(CONN_MM.MENU{thishandle}.handle.axes(n1)); end
                ...
                mx1=mean(ximage(:));mx2=mean(fontcolor);
                px0=abs(mx1-mx2);
                fontcolor1=fontcolor*px0+min(1,mx1+.5)*(1-px0); px1=abs(mx1-mean(fontcolor1));
                fontcolor2=fontcolor*px0+max(0,mx1-.5)*(1-px0); px2=abs(mx1-mean(fontcolor2));
                px=[px0 px1 px2].^4; [nill,ipx]=max(px); px(ipx)=px(ipx)+.1; px=px/max(eps,sum(px));
                fontcolor=px*[fontcolor;fontcolor1;fontcolor2];
...
%                 end
                if x>1, fontsize=fontsize+CONN_MM.MENU{thishandle}.dfont; end
                
                if CONN_gui.dounixGUIbugfix
                    CONN_MM.MENU{thishandle}.handle.text{n1}=[];
                    sstrings=cellstr(CONN_MM.MENU{thishandle}.string{n1});
                    nstrings=numel(sstrings);
                    for n2=1:nstrings
                        if CONN_MM.MENU{thishandle}.order(1)=='h', tpos=[pos(1)+(n2-1)/nstrings*pos(3) pos(2) pos(3)/nstrings pos(4)];
                        else tpos=[pos(1) pos(2)+pos(4)-n2/nstrings*pos(4) pos(3) pos(4)/nstrings];
                        end
                        ...
                        htemp=uicontrol('style','togglebutton','units','norm','position',tpos,'string',sstrings{n2},'parent',CONN_MM.gcf,'callback','conn_menumanager(''cursorup'');',...
                            'foregroundcolor',fontcolor,... %(1-CONN_MM.MENU{thishandle}.color(x,end)).*CONN_MM.MENU{thishandle}.fontcolor,... %,CONN_MM.MENU{thishandle}.fontcolor,...
                        ...
                        xextent=get(htemp,'position')+2;
                        set(htemp,'cdata',ximage(round(linspace(1,size(ximage,1),xextent(4))),round(linspace(1,size(ximage,2),xextent(3))),:));
                        if n2==1, CONN_MM.MENU{thishandle}.handle.axes(n1)=htemp; else CONN_MM.MENU{thishandle}.handle.text{n1}(n2-1)=htemp; end
                        %if CONN_MM.MENU{thishandle}.roll&&numel(changed)==CONN_MM.MENU{thishandle}.n, drawnow; end
                    end
                elseif 1
                else % back reference; to be removed in future release
                    if CONN_MM.MENU{thishandle}.order(1)=='h', ximage=repmat(ximage,[1,t0(n1)]); s=size(ximage); sx=(s(2)+1)*(2*(1:t0(n1))-1)/(2*t0(n1)); sy=(s(1)+1)/2+zeros(1,t0(n1));
                    else ximage=repmat(ximage,[t0(n1),1]); s=size(ximage); sx=(s(2)+1)/2+zeros(1,t0(n1)); sy=(s(1)+1)*(2*(1:t0(n1))-1)/(2*t0(n1)); end
                    CONN_MM.MENU{thishandle}.handle.axes(n1)=axes('units','norm','position',pos);
                    if CONN_MM.MENU{thishandle}.rotate,
                        CONN_MM.MENU{thishandle}.handle.image(n1)=image(ximage'); axis tight; axis off;
                        set(gca,'ydir','normal');
                        nm=text(sy(1),sx(1),'AAA','horizontalalignment',CONN_MM.MENU{thishandle}.horizontalalignment,'verticalalignment','middle','fontname',CONN_MM.MENU{thishandle}.fontname,'fontsize',fontsize+CONN_gui.font_offset,'fontweight',CONN_MM.MENU{thishandle}.fontweight,'rotation',90);
                        offset=-(get(nm,'extent')-[sy(1),sx(1),0,0])*[1,0;0,1;.5,0;0,.5];
                        delete(nm);
                        CONN_MM.MENU{thishandle}.handle.text{n1}=text(sy+offset(1),sx+offset(2),CONN_MM.MENU{thishandle}.string{n1},'rotation',90,'clipping','on');
                    else
                        xtimage=ximage;
                        if 0,%strcmp(CONN_MM.MENU{thishandle}.bordertype,'square')
                            bg=max(0,min(1,.25+CONN_MM.MENU{thishandle}.backgroundcolor));
                            bg2=max(0,min(1,-.25+CONN_MM.MENU{thishandle}.backgroundcolor));
                            if strcmp(CONN_MM.MENU{thishandle}.order,'horizontal')
                                xtimage([1,2],:,1)=bg(1);xtimage([1,2],:,2)=bg(2);xtimage([1,2],:,3)=bg(3);
                                xtimage([end-1,end],:,1)=bg2(1);xtimage([end-1,end],:,2)=bg2(2);xtimage([end-1,end],:,3)=bg2(3);
                                if n1==1
                                    xtimage(:,[1,2],1)=bg2(1);xtimage(:,[1,2],2)=bg2(2);xtimage(:,[1,2],3)=bg2(3);
                                end
                                if n1==CONN_MM.MENU{thishandle}.n
                                    xtimage(:,[end-1,end],1)=bg(1);xtimage(:,[end-1,end],2)=bg(2);xtimage(:,[end-1,end],3)=bg(3);
                                end
                            else
                                xtimage(:,[1,2],1)=bg2(1);xtimage(:,[1,2],2)=bg2(2);xtimage(:,[1,2],3)=bg2(3);
                                xtimage(:,[end-1,end],1)=bg(1);xtimage(:,[end-1,end],2)=bg(2);xtimage(:,[end-1,end],3)=bg(3);
                        ...
                        CONN_MM.MENU{thishandle}.handle.image(n1)=image(flipud(xtimage)); axis tight; axis off;
                        set(gca,'ydir','normal');
                        %CONN_MM.MENU{thishandle}.handle.text{n1}=annotation('textbox','units','norm','position',pos,'string',CONN_MM.MENU{thishandle}.string{n1},'edgecolor','none','horizontalalignment','center');
                        nm=text(sx(1),sy(1),'AAA','horizontalalignment',CONN_MM.MENU{thishandle}.horizontalalignment,'verticalalignment','middle','fontname',CONN_MM.MENU{thishandle}.fontname,'fontsize',fontsize+CONN_gui.font_offset,'fontweight',CONN_MM.MENU{thishandle}.fontweight);
                        offset=-(get(nm,'extent')-[sx(1),sy(1),0,0])*[1,0;0,1;.5,0;0,.5];
                        delete(nm);
                        CONN_MM.MENU{thishandle}.handle.text{n1}=text(sx+offset(1),sy+offset(2),CONN_MM.MENU{thishandle}.string{n1},'clipping','on');
                    end
                    set(CONN_MM.MENU{thishandle}.handle.text{n1},'color',fontcolor,... %(1-CONN_MM.MENU{thishandle}.color(x,end)).*CONN_MM.MENU{thishandle}.fontcolor,... %,CONN_MM.MENU{thishandle}.fontcolor,...
                        'fontname',CONN_MM.MENU{thishandle}.fontname,...
                        'fontsize',fontsize+CONN_gui.font_offset,...
                        'fontunits','pixels',...
                        'horizontalalignment',CONN_MM.MENU{thishandle}.horizontalalignment,...
...
if params.order(1)=='h', p2(1)=p2(1)/params.n; 
else p2(2)=p2(2)/params.n; 
end
...
%         bg=params.backgroundcolor;
        ...
        t=max(0,min(1,t));
        %t=max(0,min(1,t)).^.5;
        t=max(0,t-.05).^.125;%.5+.5*tanh(10*(t-.1));%t.^.05;%.25;
        t(tx>=kx-2)=0;
        t2=repmat(linspace(1,0,size(t,1))'.^2,1,size(t,2)).*t;%t2(ty>=ky-2|tx>=kx-2)=1;
        %bg=[0 0 0];%min(1,4*params.backgroundcolor);
        bg=params.backgroundcolor;
        t={t/2,t2,t/2};
    case 'square'
        t=.5+zeros(size(tx));
        t2=.75+0*repmat(linspace(1,-1,size(t,1))'.^6,1,size(t,2)).*t;%t2(ty>=ky-2|tx>=kx-2)=1;
...
for x=1:3, 
    if any(params.color(x,:)<0)
        ...
        pos=[p1 p2];
        if params.order(1)=='h', pos(1)=pos(1)+pos(3)*(n-1);
        else pos(2)=pos(2)+pos(4)*(params.n-n);
        end
