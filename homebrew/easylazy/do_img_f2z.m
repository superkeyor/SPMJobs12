function do_img_f2z(folders)
    % input a folder or folders, look for spmF_xxx.nii
    % read for df from the F image decription
    % call built-in f2z to convert from F to Z image
    ...
    for j = 1:length(folders)
        ...
        for i = 1:length(P)
            ...
            fvalue = spm_read_vols(P{i});
            % df can be found in P.descrip, I think.
            df = regexp(oP.descrip,'\{F_\[\d+\.\d+\,\d+\.\d+\]\}','once','match');
            df = regexp(df,'\d+\.\d+','match');
            df1 = df{1};  df2 = df{2}; 
            df1 = str2double(df1); df2 = str2double(df2);
...
% jerry:
% https://www.jstatsoft.org/article/view/v023c01/
% see also https://raw.githubusercontent.com/jmcarp/fmri-pipe/master/f2z.m
% https://raw.githubusercontent.com/jmcarp/fmri-pipe/master/t2z.m
