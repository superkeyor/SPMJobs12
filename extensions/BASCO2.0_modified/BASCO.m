...
for isubj=1:NumSubj % loop over subjects  %%%%%%%%%%%%%%%%%%%%%
    ...
    matlabbatch{1}.cfg_basicio.file_dir.dir_ops.cfg_mkdir.name = outdirname;
    ...
    for irun=1:NumRuns % loop over runs
        ...
        for icond=1:AnaDef.NumCond % loop over conditions
            ...
            % hack by Jerry start
            if NumOnsets==1 && onsets(icond,1)<0
                handles.anaobj{isubj}.Ana{1}.AnaDef.RegCond.Run{irun}.Cond{icond} = [];
                continue; 
            end % skip a cond for this run
            % hack by Jerry end
...
figure('Name','');
% estimated is estimated with marsbar
% mean is simply mean/average within roi witout marsbar re-estimatation
...
legend('re-estimated on ROI level','simply averaged on voxel level (mean)');
guidata(hObject, handles);
