...
answ=varargin;
if any(strcmp(answ,'-spm')), hfig=spm_figure('FindWin','Graphics');if isempty(hfig)||~ishandle(hfig), return; end; answ=answ(~strcmp(answ,'-spm'));
else hfig=gcf;
end
...
if any(cellfun(@(x)any(strcmp(x,{'-mosaic','-column','-row','-mosaic8'})),answ(charansw)))
    idx=charansw(find(cellfun(@(x)any(strcmp(x,{'-mosaic','-column','-row','-mosaic8'})),answ(charansw)),1));
...
if ~isempty(answ)
    ...
    if dogui&&~isempty(dir(filename))
        tansw=questdlg('Overwrite existing file?','Output file already exist','Yes','No','Yes');
