...
% 
% Select the measure that you want to explore in the 'Outcome variables' list
% 
% Select AllSubjects in the 'Predictor variables' list to simply display 
% the values of your measure, or select any combination of predictor variables 
% there to perform a second-level general linear analysis of your measure of interest
% 
...
%     Setup->Covariates->First-level tab, by clicking on the 'subject-level 
%     aggregate' button
% 
...
if CONN_x.Setup.nsubjects==1, uiwait(errordlg('Single-subject second-level analyses not supported (only population-level inferences via random-effect analyses available). Please add more subjects before proceeding.','')); end
...
if ~nargin||isempty(icovariates2), 
    icovariates1=1:numel(CONN_x.Setup.l2covariates.names)-1; 
    icovariates2=1:numel(CONN_x.Setup.l2covariates.names)-1; 
    fulltype=1; 
else
    icovariates1=setdiff(1:numel(CONN_x.Setup.l2covariates.names)-1,icovariates2); 
    ...
    mnames=regexprep(mnames,'^Dynamic\s*','');
...
if fulltype, 
    ...
    conn_menu('frame',[fs0 fs],'');%'SECOND-LEVEL MODEL');
    conn_menu('pushbutton',[fs0 0 0]+[fs fs].*[.90,.95,.10,.05],'','Help','','conn(''gui_help'',''help'',''conn_calculator.m'');'); 
    ht1=conn_menu('listbox',[fs0 0 0]+[fs fs].*[.02,.4,.45,.5],'Predictor variables',CONN_x.Setup.l2covariates.names(icovariates1),'Select subject effect(s) characterizing second-level analysis model / independent variables',@(varargin)conn_calculator_update);
    ht3=conn_menu('edit',[fs0 0 0]+[fs fs].*[.02,.28,.45,.05],'Between-subjects contrast','1','<HTML>Define desired contrast across selected subject-effects<br/> - enter contrast vector/matrix with as many elements/columns as subject-effects selected <br/> - right-click or use the list below to see a list of standard contrasts for the selected subject-effects <br/> - enter multiple rows separated by <b>;</b> (semicolon) for OR conjunction (multivariate test) of several contrasts</HTML>',@(varargin)conn_calculator_update);
    ht2=conn_menu('listbox',[fs0 0 0]+[fs fs].*[.48,.4,.50,.5],'Outcome variables',CONN_x.Setup.l2covariates.names(icovariates2),'Select outcome measure(s) / dependent variables',@(varargin)conn_calculator_update);
    ht4=conn_menu('edit',[fs0 0 0]+[fs fs].*[.48,.28,.50,.05],'Between-measures contrast','1','<HTML>Define desired contrast across selected outcome measures <br/> - enter contrast vector/matrix with as many elements/columns as outcome measures selected <br/> - right-click or use the list below to see a list of standard contrasts for the selected outcome measures<br/> - enter multiple rows separated by <b>;</b> (semicolon) for OR conjunction (multivariate test) of several contrasts</HTML>',@(varargin)conn_calculator_update);
    set(ht1,'max',2,'value',1);
    set(ht2,'max',2,'value',length(icovariates2));
    ht5=conn_menu('text',[fs0 0 0]+[fs fs].*[.02,.02,.96,.10],'Statistics','','Effect size and statistics for selected second-level analysis');
else
    fs0=[.15 .25];
    fs=[.55 .65];
    fs0b=[fs0(1)+fs(1)+.05 fs0(2)];
    ...
    %thfig=figure('units','norm','position',[.15,.3,.8,.5],'name','conn second-level calculator','numbertitle','off','menubar','none','color',[1 1 1]);
    conn_menu('frame',[fs0 fs],'');%'SECOND-LEVEL MODEL');
    ht1=conn_menu('listbox',[fs0 0 0]+[fs fs].*[.02,.25,.30,.65],'Subject effects',CONN_x.Setup.l2covariates.names(icovariates1),'Select subject effect(s) characterizing second-level analysis model / independent variables',@(varargin)conn_calculator_update);
    ...
    ht4c=conn_menu('edit',[fs0 0 0]+[fs fs].*[.34,.10,.30,.05],'Between-conditions contrast','1','<HTML>Define desired contrast across selected conditions <br/> - enter contrast vector/matrix with as many elements/columns as selected conditions <br/> - right-click or use the list below to see a list of standard contrasts for the selected condition<br/> - enter multiple rows separated by <b>;</b> (semicolon) for OR conjunction (multivariate test) of several contrasts</HTML>',@(varargin)conn_calculator_update);
    ht2=conn_menu('listbox',[fs0 0 0]+[fs fs].*[.66,.25,.30,.65],'Dynamic FC measures',umnames,'<HTML>Select outcome measure(s) <br/> - <i>baseline</i> measures represent average connectivity scores for each subject/condition along each connectivity dimension/factor <br/> - <i>modulation</i> measures represent temporal variability in connectivity scores for each subject/condition along each connectivity dimension/factor <br/> - <i>rate of change</i> measures represent average scan-to-scan changes in connectivity scores for each subject/condition along each connectivity dimension/factor</HTML>',@(varargin)conn_calculator_update);
    ...
    ht5=conn_menu('text',[fs0 0 0]-[0 .15 0 0]+[fs fs].*[.02,.0,.96,.10],'Analysis results','','Effect size and statistics for selected second-level analysis');
end
handleplot=[];
% try
% if domacbugfix,
...
conn_calculator_update;
    ...
    function conn_calculator_update(newsortplot,newspacing,newlabels,newshownull,newchangedvars)
        ...
        set(ht5,'string',str);
        conn_contrasthelp(ht3,get(ht1,'string'),get(ht1,'value'),all(ismember(X1(:,get(ht1,'value')),[0 1]),1)+2*all(ismember(X1(:,get(ht1,'value')),[-1 1]),1));
        if fulltype
            conn_contrasthelp(ht4,get(ht2,'string'),get(ht2,'value'),all(ismember(X2(:,get(ht2,'value')),[0 1]),1)+2*all(ismember(X2(:,get(ht2,'value')),[-1 1]),1));
        else
            conn_contrasthelp(ht4c,ucnames,ncov2c,[]);
            conn_contrasthelp(ht4,umnames,ncov2m,[]);
        end
        ...
        for n=1:size(ccov2,1)
            ...
            set(handle,'units','norm','position',[fs0b 0 0]+[fsb fsb].*[.1 .2+.75-.75*n/size(ccov2,1) .8 .6/size(ccov2,1)],'xcolor',.7*[1 1 1],'ycolor',.7*[1 1 1],...
                'color',CONN_gui.backgroundcolorA,'box','off',...
