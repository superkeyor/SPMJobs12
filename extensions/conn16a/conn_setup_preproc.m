...
%       functional_slicetime: 'sliceorder' 'ta'
    ...
    switch(regexprep(lower(STEP),'^run_|^update_|^interactive_',''))
        ...
        case 'structural_normalize'
            if 0, % deprecated in favor of explicit center step
                jsubject=0;
                for isubject=1:numel(subjects), % center first
                    nsubject=subjects(isubject);
                    if CONN_x.Setup.structural_sessionspecific, nsess=CONN_x.Setup.nsessions(min(numel(CONN_x.Setup.nsessions),nsubject)); else nsess=1; end
                    for nses=1:nsess
                        jsubject=jsubject+1;
        ...
        case 'functional_slicetime'
            ...
            for isubject=1:numel(subjects),
                ...
                if isubject<=numel(sliceorder_all), sliceorder=sliceorder_all{min(numel(sliceorder_all),nsubject)}; end
                        ...
                        for n=1:numel(temp)
                            ...
                            if isnan(treorient)
                                if isempty(translation)
                                    ...
                                    try, R=eye(4);R(1:3,4)=translation; save(conn_prepend('centering_',temp{n},'.mat'),'R','-mat'); end
                                end
                                M{n}(1:3,4)=M{n}(1:3,4)+translation;
                            else
                                R=[treorient zeros(3,1); zeros(1,3) 1];
                                M{n}=R*M{n};
                                if n==1, try, save(conn_prepend('reorient_',temp{n},'.mat'),'R','-mat'); end; end
                            end
                        end
                        for n=1:numel(temp)
                ...
                for isubject=1:numel(subjects),
                    ...
                    for nses=1:nsess
                        ...
                        if isnan(treorient)
                            ...
                            try, R=eye(4);R(1:3,4)=translation; save(conn_prepend('centering_',temp,'.mat'),'R','-mat'); end
                        else
                            R=[treorient zeros(3,1); zeros(1,3) 1];
                            M=R*M;
                            try, save(conn_prepend('reorient_',temp,'.mat'),'R','-mat'); end
                        end
