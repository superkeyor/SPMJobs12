...
if ischar(options),
    ...
    while ~isempty(optionsnow),
        switch(lower(optionsnow)),
            ...
            case 'all',             disp(['CONN: RUNNING ALL ANALYSIS STEPS']); conn_process([0:4,4.5,5,1.5,2,[6:11 13 13.5 14:15]]);
            ...
            case 'analyses_vvmvpa',     disp(['CONN: RUNNING ANALYSIS STEP (voxel-to-voxel MVPA analyses)']); conn_process([9,13.1],varargin{:});
            ...
            case 'analyses_dyn',    disp(['CONN: RUNNING DYNAMIC CONNECTIVITY STEP']); conn_process([9,13.5],varargin{:});
...
if any(options==4) && any(CONN_x.Setup.steps([1,2,3])) && ~(isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'steps')&&~any(CONN_x.gui.steps([1,2,3]))),
	...
	for nsub=validsubjects,
        ...
        else THRall=.5;  % threshold for white/CSF probability maps before erosion
        end
        if isfield(CONN_x.Setup,'cwthreshold')&&numel(CONN_x.Setup.cwthreshold)>1, ERODEall=CONN_x.Setup.cwthreshold(2:2:end);
        else ERODEall=1; % erosing level for white/csf masks (voxels) (set to 0 for no erosion)
        end
	...
	for nsub=validsubjects,
        ...
        if strcmp(lower(REDO),'yes')||any(missingdata),
            ...
            for nses=1:nsess, % loads all ROI COV COND data for this subject
                ...
                [X{nses},nill,nill,nill,Xnames{nses}]=conn_designmatrix(confounds,X1{nses},X2{nses});
    ...
    for nsub=validsubjects,
        for ncondition=validconditions,
            ...
            if strcmp(lower(REDO),'yes')||(isempty(dir(filename))),
                ...
                try
                    temp1=[.7:.1:.9];
                    temp2=ceil(interp1([0;cumsum(D.^2)/sum(D.^2)],0:numel(D),temp1));
                    ...
                    for slice=1:Y.matdim.dim(3),
                        ...
                        if ~isempty(idxt),
                            ...
                            conn_write_slice(Yout_A,Q2,slice); % Y = Q1*sqrtm(D)*R1'; (SVD decomposition)
                                                               ...
                                                               % Cxx = Y'*Y = R1*D*R1' = Q2'*Q2 (correlation matrix)
                        end
                        n=n+1;
                        conn_waitbar(n/N,h,sprintf('Subject %d Condition %d',nsub,ncondition));
                    end
...
%                         end
                        ...
                        elseif ~isempty(regexp(CONN_x.Analyses(ianalysis).modulation,'^Dynamic factor \d+$')), % Dyn FC
                            filename=fullfile(CONN_x.folders.firstlevel_dyn,['dyn_Subject',num2str(nsub,'%03d'),'.mat']);
                            xmod=load(filename);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if any(options==13|options==13.1) && any(CONN_x.Setup.steps([3])) && (nargin<=1||any(varargin{1}==0)) && ~(isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'steps')&&~any(CONN_x.gui.steps([3]))),
    warning('off','MATLAB:DELETE:FileNotFound');
    [path,name,ext]=fileparts(CONN_x.filename);
    filepath=CONN_x.folders.preprocessing;
    ...
    nmeasures1=nmeasures-nmeasures2;    
    ...
    if nmeasures2>0 && ~isequal(validsubjects,1:CONN_x.Setup.nsubjects), 
        if isfield(CONN_x,'pobj')&&isstruct(CONN_x.pobj)&&isfield(CONN_x.pobj,'subjects'), 
            disp('group-level factorization analyses parallelization not yet available (run locally or as a single job instead). Skipping these analyses');
            nmeasures2=0;
        else
            %validsubjects=1:CONN_x.Setup.nsubjects; %note: force group-level analyses to always include all subjects?
        end
    end
    if nmeasures2>0, %&& isequal(validsubjects,1:CONN_x.Setup.nsubjects),% MVPA
        isnew=isnew0;
%         MAXDIMS=256;
            ...
            for slice=1:Y1.size.Ns,
                for isub=1:numel(validsubjects)
                    ...
                    for ivalidcondition=1:numel(validconditions),
                        ...
                        if nsub==1&&ncondition==validconditions(1), x=zeros([size(temp,2),NdimsIn,numel(validsubjects),numel(validconditions)]); end
                        if ~isempty(temp)
                ...
                if ismtxC
                    ...
                    for n=1:numel(blockcolumns), conn_mtx('addtoblock',C,n,x'*x(:,blockcolumns{n})); end
                else C=C+x'*x;
                end
            end
                ...
                if strcmp(lower(REDO),'yes')||isnew(nmeasure)
                    ...
                    if NdimsIn>thisNdimsIn,
                        if ismtxC,
                            ...
                            C=conn_mtx('zerorows',C,conn_bsxfun(@plus,NdimsIn*(0:CONN_x.Setup.nsubjects*numel(validconditions)-1)',thisNdimsIn+1:NdimsIn));
                        else
                        ...
                        for slice=1:Y1.size.Ns,
                            for isub=1:numel(validsubjects)
                                ...
                                for ivalidcondition=1:numel(validconditions)
                                    ...
                                    if isub==1&&ncondition==validconditions(1), x=zeros([size(temp,2),NdimsIn,numel(validsubjects),numel(validconditions)]); end
                                    if ~isempty(temp)
                                        x(:,1:min(size(temp,1),NdimsIn),isub,ivalidcondition)=temp(1:min(size(temp,1),NdimsIn),:)';
                                    end
                                end
                    ...
                    elseif measures.measuretype{nmeasure}==3||measures.measuretype{nmeasure}==4 % group-ICA, group-PCA
                        ...
                        for isub=1:numel(validsubjects)
                            ...
                            for ivalidcondition=1:numel(validconditions)
                                ...
                                conn_waitbar((nsub+ivalidcondition/numel(validconditions)-1)/numel(validsubjects),h2,sprintf('Condition %d Subject %d',ivalidcondition,nsub));
                            end
                        ...
                        clear temp_out;
                        for ndim=1:NdimsOut
                            filename=fullfile(filepathresults,['PCA_Measure',num2str(imeasure(nmeasure),'%03d'),'_Component',num2str(ndim,'%03d'),'.nii']);
                            try, delete(filename); end
                        ...
                        if measures.measuretype{nmeasure}==3 % group-ICA
                            ...
                            if issurface % sorting for surfaces not implemented, reverts to default (not sorting)
                                ...
                                if conn_existfile(mask)
                                    ...
                                    fprintf(fh,'%s\n',mat2str(vY));
                                    for n=1:NdimsOut, fprintf(fh,'ICA %d: best match sortICA %d (%f)\n',n,idxmaxvY(idx(n)),maxvY(idx(n))); end
                                    fclose(fh);
                                end
                            end
                            if isempty(idx)
                            ...
                            clear temp_out;
                            for ndim=1:NdimsOut
                                filename=fullfile(filepathresults,['ICA_Measure',num2str(imeasure(nmeasure),'%03d'),'_Component',num2str(ndim,'%03d'),'.nii']);
                                try, delete(filename); end
                            ...
                            filename=fullfile(filepathresults,['ICA.ROIs.nii']);
                            spm_file_merge(temp_out,filename);
                            fh=fopen(conn_prepend('',filename,'.txt'),'wt');
                            for ndim=1:NdimsOut, fprintf(fh,'%d\n',ndim);end
                            fclose(fh);
                        ...
                        for nsub=1:CONN_x.Setup.nsubjects
                            ...
                            for ivalidcondition=1:numel(validconditions)
                                ...
                                if ~isempty(isub)
                                    ...
                                    y2=pinv(wqc*wqc')*wqc(:,1:size(y1,1))*y1*size(y1,2);
                                end
                                for ndim=1:NdimsOut
                                    filename=fullfile(filepathresults,['BETA_Subject',num2str(nsub,'%03d'),'_Condition',num2str(icondition(ncondition),'%03d'),'_Measure',num2str(imeasure(nmeasure),'%03d'),'_Component',num2str(ndim,'%03d'),'.nii']);
...
%         conn_waitbar('close',h);
    ...
    if ~isfield(CONN_x.vvAnalyses,'measures')||isempty(CONN_x.vvAnalyses.measures), CONN_x.vvAnalyses.measures=conn_v2v('list_extended',CONN_x.vvAnalyses.regressors);
    else % remove from list of displayed results other analyses by the same name
        newmeasures=conn_v2v('list_extended',CONN_x.vvAnalyses.regressors);
        ko=[]; for n1=1:numel(newmeasures), ko=[ko conn_v2v('match_existing',newmeasures{n1})]; end
        if any(ko), CONN_x.vvAnalyses.measures=CONN_x.vvAnalyses.measures(setdiff(1:numel(CONN_x.vvAnalyses.measures),ko)); end
        CONN_x.vvAnalyses.measures=[CONN_x.vvAnalyses.measures newmeasures];
    end
    try
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if any(options==13.5) && any(CONN_x.Setup.steps([4])) && ~(isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'steps')&&numel(CONN_x.gui.steps)>3&&~any(CONN_x.gui.steps([4]))),
    [path,name,ext]=fileparts(CONN_x.filename);
    filepath=CONN_x.folders.preprocessing;
    filepathresults=CONN_x.folders.firstlevel_dyn;
	nconditions=length(CONN_x.Setup.conditions.names)-1;
    if isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'subjects'), validsubjects=CONN_x.gui.subjects; else validsubjects=1:CONN_x.Setup.nsubjects; end
    if isfield(CONN_x,'pobj')&&isstruct(CONN_x.pobj)&&isfield(CONN_x.pobj,'subjects'), validsubjects=CONN_x.pobj.subjects; end
    if isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'conditions'), validconditions=CONN_x.gui.conditions; else validconditions=1:length(CONN_x.Setup.conditions.names)-1; end
    icondition=[];isnewcondition=[];for ncondition=validconditions,[icondition(ncondition),isnewcondition(ncondition)]=conn_conditionnames(CONN_x.Setup.conditions.names{ncondition},'+'); end
    %h=conn_waitbar(0,['Step ',num2str(sum(options<=13.5)),'/',num2str(length(options)),': dynamic temporal-components estimation']);
    ...
    roinames=CONN_x.dynAnalyses.regressors.names;  % cell-array of ROI names (empty to use all ROIs)
    window=CONN_x.dynAnalyses.window;
    
    if 0, %CONN_x.dynAnalyses.analyses==1||CONN_x.dynAnalyses.analyses==3 % dynamic connectivity measures
        
        REDO='Yes'; filename=fullfile(filepathresults,['sw_Subject',num2str(validsubjects(1),'%03d'),'.mat']);
        if ~isempty(dir(filename)),if isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'overwrite'), REDO=CONN_x.gui.overwrite; else, REDO=questdlg('Overwrite existing subject results?','','Yes', 'No', 'Yes');end; end
        h=conn_waitbar(0,['Step ',num2str(sum(options<=13.5)),'/',num2str(length(options)),': dynamic connectivity estimation']);
        if strcmp(lower(REDO),'yes')||isempty(dir(filename))
            REDO='Yes';
            COND_names={};
            COND_weights={};
            for nsub=validsubjects,
                ...
                X1=load(filename);
                if isempty(roinames), roinames=X1.names(~ismember(X1.names,{'White Matter','CSF'})); end
                ...
                R=zeros(size(y,2),size(y,2),size(y,1));
                xX=reshape(xfilter*reshape(conn_bsxfun(@times,y,permute(y,[1,3,2])),size(y,1),[]),size(y,1),size(y,2),size(y,2));
                for n=1:size(y,1)
                    cxx=shiftdim(xX(n,:,:));
                    r=(cxx+cxx')./bsxfun(@plus,diag(cxx),diag(cxx)');
                    R(:,:,n)=r;
                end
                
                filename=fullfile(filepathresults,['sw_Subject',num2str(nsub,'%03d'),'.mat']);
                ROInames=roinames;
                data_sessions=IDX_session;
                save(filename,'R','ROInames','data_sessions');
                conn_waitbar(mean(nsub>=validsubjects),h,sprintf('Subject %d',nsub));
            end
            CONN_x.dynAnalyses.sources=roinames;
        else
            conn_waitbar(1,h);
        end
        conn_waitbar('close',h);
    ...
    if 1, %CONN_x.dynAnalyses.analyses==2||CONN_x.dynAnalyses.analyses==3 % dynamic factor analysis
        do=true;
        if ~isequal(validsubjects,1:CONN_x.Setup.nsubjects), 
            if isfield(CONN_x,'pobj')&&isstruct(CONN_x.pobj)&&isfield(CONN_x.pobj,'subjects'), disp('Dynamic factor analysis parallelization not yet available (run locally or as a single job instead). Skipping these analyses'); do=false;
            else
                %validsubjects=1:CONN_x.Setup.nsubjects; %note: force group-level analyses to always include all subjects?
                %do=false;
            end
        end
        if do, %isequal(validsubjects,1:CONN_x.Setup.nsubjects), 
            ...
            if CONN_x.dynAnalyses.output(2)
                ...
                for ncomp=1:Ncomponents
                    for n=1:numel(COND_names)
                        ...
                        H_avg_weighted=accumarray(IDX_subject,tempH.*tempW,[CONN_x.Setup.nsubjects,1],@sum,nan)./w;
                        H_std_weighted=max(0,accumarray(IDX_subject,H(:,ncomp).^2.*tempW,[CONN_x.Setup.nsubjects,1],@sum,nan)./w - (accumarray(IDX_subject,H(:,ncomp).*tempW,[CONN_x.Setup.nsubjects,1],@sum,nan)./w).^2);
...
if any(options==16) && any(CONN_x.Setup.steps([2,3])) && ~(isfield(CONN_x,'gui')&&isstruct(CONN_x.gui)&&isfield(CONN_x.gui,'steps')&&~any(CONN_x.gui.steps([2,3]))),
    ...
    if state==2
        ...
        filepathresults1=CONN_x.folders.firstlevel_vv;
    end
                ...
                if numel(nconditions)>1&&size(cconditions,1)==1
                    for n1=1:numel(nsources)
                        ...
                        for n0=1:numel(nconditions)
                            ...
                            if cconditions(n0)==1, tcontrastname=[tcontrastname,'+',contrastname{n1,n0}];
                            elseif cconditions(n0)==-1, tcontrastname=[tcontrastname,'-',contrastname{n1,n0}];
                            else tcontrastname=[tcontrastname,num2str(cconditions(n0)),'*',contrastname{n1,n0}];
                            end
                ...
                if numel(nsources)>1&&size(csources,1)==1
                    for n0=1:size(filename,2)
                        ...
                        for n1=1:numel(nsources)
                            ...
                            if csources(n1)==1, tcontrastname=[tcontrastname,'+',contrastname{n1,n0}];
                            elseif csources(n1)==-1, tcontrastname=[tcontrastname,'-',contrastname{n1,n0}];
                            else tcontrastname=[tcontrastname,num2str(csources(n1)),'*',contrastname{n1,n0}];
                            end
