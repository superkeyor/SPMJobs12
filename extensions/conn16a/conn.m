...
if nargin<1,
    ...
    conn_backgroundcolor=.75*[.03 .12 .2];            % backgroundcolor
    ...
    uicontrol('style','text','units','norm','position',[.6 .2 .4 .5],'string',[connver],'fontunits','norm','fontsize',.075,'horizontalalignment','left','foregroundcolor',[0 0 0]+(mean(CONN_gui.backgroundcolor)<.5),'backgroundcolor',CONN_gui.backgroundcolor);
	...
	CONN_h.menus.m_setup_07d=conn_menumanager([],	'n',6,...
                                    ...
                                    'toggle',0,...
                                    'roll',1,...
									'position',[.14,.955-4.5*.045-6*.045,.099,6*.045],...
									'fontsize',8,...
                                    'bordertype','square',...
	...
	CONN_h.menus.m_setup_07b=conn_menumanager([],	'n',5,...
									'string',{'Search','Updates','Documentation','Sample data','Web'},...
									'help',{'Search on a database of support questions','Check for software updates','See documentation','Download and process sample dataset','Additional information',''},...
                                    'order','vertical',...
                                    'toggle',0,...
                                    'roll',1,...
									'position',[.09,.955-5*.045,.099,5*.045],...
									'fontsize',8,...
                                    'bordertype','square',...
									'callback',{{@conn_msghelp},{@conn_update,connver},CONN_h.menus.m_setup_07c,{@conn_batch_workshop_nyudataset},CONN_h.menus.m_setup_07d} );
...
% 									'callback',{{@conn,'gui_setup_covariates'},{@conn,'gui_setup_covariates'}} );
	...
	CONN_h.menus.m_results_03a=conn_menumanager([], 'n',3,...
									'string',{'Components','Dynamic properties','Summary display'},...
									'help',{'Define/explore second-level analyses of dynamic components/circuits','Define/explore second-level analyses of dynamic properties of connectivity-modulation timeseries','Summary display of dynamic connectivity analyses'},...
                                    'order','vertical',...
	...
	switch(lower(varargin{1})),
        case 'init',
            CONN_x=struct('name',[],'filename','','gui',1,'state',0,'ver',connver,...
                ...
                'folders',struct('rois',fullfile(fileparts(which(mfilename)),'rois'),'data',[],'preprocessing',[],'firstlevel',[],'firstlevel_vv','firstlevel_dyn','secondlevel',[]),...
                'Setup',struct(...
                ...
                'Analyses',struct(...
                 ...
                 'weight',[]),...
                'Analysis',1,...
                'dynAnalyses',struct('regressors', struct('names',{{}}),'variables', struct('names',{{}}),'Ncomponents',[20],'condition',1,'analyses',3,'window',10,'output',[1 1 0]),...
                'vvAnalyses',struct(...
                 'measurenames',{{}},...
                ...
                if ~saveas
                    ...
                    if isfield(CONN_x,'vvAnalyses')
                        if isfield(CONN_x.vvAnalyses,'measurenames')
                            filemeasurenames=fullfile(CONN_x.folders.firstlevel_vv,'_list_measures.mat');
        ...
        case 'gui_setup_preproc'
            ...
            if ok==2
                conn gui_setup; 
                conn_msgbox({'Preprocessing finished correctly; Output files imported.'},'');
                %conn save; 
            elseif ok==1
                conn_msgbox({'Preprocessing finished correctly'},'');
            elseif ok<0
        ...
        case 'run_process'
            ...
            if conn_questdlg('Choose processing steps',[],CONN_x.Setup.steps(1:3),false,[],true,true,true);
                conn_menumanager clf;
                conn_menuframe;
                conn_menumanager([CONN_h.menus.m_setup_06,CONN_h.menus.m0],'on',1);
                ...
                if CONN_x.gui.parallel~=0, 
                    ...
                    conn save;
                    conn_jobmanager('submit',psteps,subjects,[],CONN_x.gui);
        ...
        case 'gui_setup',
            ...
            switch(state),
                case 1, %basic
                    boffset=[.15 -.05 0 0];
                    if nargin<2,
                        conn_menu('frame',boffset+[.19,.36,.27,.44],'Basic info');
						...
						CONN_h.menus.m_setup_00{2}=conn_menu('edit',boffset+[.2,.6,.25,.04],'Number of sessions',num2str(CONN_x.Setup.nsessions,'%1.0f '),'<HTML>Number of scanning sessions per subject <br/> - enter a single number if the same number of scanning sessions were acquired for each subject, or a different number per subject otherwise (e.g. 2 2 3)</HTML>','conn(''gui_setup'',2);');
						CONN_h.menus.m_setup_00{3}=conn_menu('edit',boffset+[.2,.5,.25,.04],'Repetition Time (seconds)',mat2str(CONN_x.Setup.RT),'<HTML>Sampling period of fMRI data (time between two consecutive whole volume acquisitions<br/> - enter a single number if the same TR was used for each subject, or a different number per subject otherwise</HTML>','conn(''gui_setup'',3);');
                ...
                case 3, %functional
                    boffset=[.02 .06 0 0];
                    if nargin<2,
                        conn_menu('frame',boffset+[.19,.20,.50,.60],'Functional data for voxel-level analyses');
                        ...
                        tmp=conn_menu('popup0',boffset+[.60,.80,.09,.05],'',{'(set 0)'},'','');
						CONN_h.menus.m_setup_00{1}=conn_menu('listbox',boffset+[.200,.27,.075,.48],'Subjects','','Select subject(s)','conn(''gui_setup'',1);');
						CONN_h.menus.m_setup_00{2}=conn_menu('listbox',boffset+[.275,.27,.075,.48],'Sessions','','Select session','conn(''gui_setup'',2);');
						CONN_h.menus.m_setup_00{3}=conn_menu('filesearch',[],'Select functional data files','*.img; *.nii; *.gz','',{@conn,'gui_setup',3},'conn(''gui_setup'',4);');
						CONN_h.menus.m_setup_00{4}=conn_menu('pushbutton2',boffset+[.35,.71,.34,.09],'','','','conn(''gui_setup'',4);');
						CONN_h.menus.m_setup_00{5}=conn_menu('image',boffset+[.36,.25,.31,.45]);
                        set([CONN_h.menus.m_setup_00{4}],'visible','off'); conn_menumanager('onregion',[CONN_h.menus.m_setup_00{4}],1,boffset+[.35,.25,.34,.55]);
                        CONN_h.menus.m_setup_00{14}=conn_menu('popup0',boffset+[.20,.20,.20,.05],'',{'<HTML><i> - functional tools:</i></HTML>','Slice viewer','Check functional/anatomical coregistration','Check functional/MNI coregistration','Apply individual preprocessing step','Move set-N to set-0'},'<HTML> - <i>slice viewer</i> displays first functional volume slices<br/> - <i>check registration</i> checks the coregistration of the selected subject functional/anatomical files <br/> - <i>preprocessing</i> runs individual preprocessing step on functional volumes (e.g. realignment, slice-timing correction, etc.)<br/> - <i>move set-N to set-0</i> reassigns set-0 functional volumes to match the current set of functional volumes defined in set-N</HTML>','conn(''gui_setup'',14);');
                        nset=1;
                        newdelete={'<HTML><i>new</i></HTML>','<HTML><i>delete</i></HTML>'}; if numel(CONN_x.Setup.roifunctional)==1, newdelete=newdelete(1); end
                        CONN_h.menus.m_setup_00{7}=conn_menu('popup0',boffset+[.60,.11,.09,.05],'',[arrayfun(@(n)sprintf('(set %d)',n),1:numel(CONN_x.Setup.roifunctional),'uni',0),newdelete],'<HTML>Define one or multiple sets of functional data for ROI BOLD-signal timeseries extraction<br/> - Multiple sets may be useful when you have a combination of ROIs defined in different spaces (e.g. MNI- or subject- space)</HTML>','conn(''gui_setup'',7);');
                        analysistypes={'Same files','Other: same filenames without leading ''s'' (SPM convention for unsmoothed volumes)','Other: manually define other filename conventions','Other: manually defined set of functional files'};
                        if CONN_x.Setup.roifunctional(nset).roiextract<=3, analysistypes=analysistypes(1:3); end
                        CONN_h.menus.m_setup_00{6}=conn_menu('popup',boffset+[.20,.04,.48,.05],'',analysistypes,'<HTML>Select if you would like to extract ROI BOLD-signal timeseries from the same functional volumes as above or from a different set of functional volumes/files<br/> - note: Typically Voxel-level BOLD timeseries are extracted from spatially-smoothed functional volumes while ROI-level BOLD timeseries are extracted from  <br/> the original -unsmoothed- volumes in order to minimize spillage from neighbouring ROIs</HTML>','conn(''gui_setup'',6);');
                        ...
                        switch(varargin{2}),
                            ...
                            case 7,
                                ...
                                elseif nset==numel(CONN_x.Setup.roifunctional)+2, %remove
                                    str=arrayfun(@(n)sprintf('set %d',n),1:numel(CONN_x.Setup.roifunctional),'uni',0);
                                    ...
                                    if ~isempty(nset)
                                        if numel(nset)==numel(CONN_x.Setup.roifunctional), uiwait(warndlg({'At least one set must remain',' ','Set deletion canceled'},''));
                                ...
                                newdelete={'<HTML><i>new</i></HTML>','<HTML><i>delete</i></HTML>'}; if numel(CONN_x.Setup.roifunctional)==1, newdelete=newdelete(1); end
                                set(CONN_h.menus.m_setup_00{7},'string',[arrayfun(@(n)sprintf('(set %d)',n),1:numel(CONN_x.Setup.roifunctional),'uni',0),newdelete],'value',nset);
                            case 14,
...
%                                             try
                                                for nses=nsess(:)',
                                                    ...
                                                    for unsmoothedvolumes=0:numel(CONN_x.Setup.roifunctional)
...
%                                             end
                                        ...
                                        return;
                                    case 5, % preprocessing 
                                        set(CONN_h.menus.m_setup_00{14},'value',1);
                                        conn('gui_setup_preproc','select','functional');
                                        return;
                                    case 6, % move set-N to set-0
                                        set(CONN_h.menus.m_setup_00{14},'value',1);
                                        nalt=listdlg('liststring',arrayfun(@(n)sprintf('set %d',n),1:numel(CONN_x.Setup.roifunctional),'uni',0),'selectionmode','single','initialvalue',1,'promptstring',{'Select set number:','(set-0 functional data will be reassigned to the current functional volumes specified in the selected set)'},'ListSize',[300 200]);
                                        ...
                                        if ishandle(hmsg), delete(hmsg); end
                                        if err||strcmp(questdlg('Set-0 functional volumes successfully reassigned','','Ok','Undo','Ok'),'Undo'), CONN_x.Setup.functional=bak1; CONN_x.Setup.nscans=bak2; end
                                        if err, uiwait(errordlg({sprintf('Error: set-%d data for subject %d session %d not found.\n',nalt,nsub,nses),'Operation canceled'},'')); end
                                    otherwise
                                        set(CONN_h.menus.m_setup_00{14},'value',1);
                                        return;
                                end
...
%                                 CONN_x.Setup.normalized=normalized;
                ...
                case 2, %structural
                    boffset=[.02 0 0 0];
                    if nargin<2,
                        conn_menu('frame',boffset+[.19,.15,.45,.72],'Structural data');
						...
						CONN_h.menus.m_setup_00{4}=conn_menu('pushbutton2', boffset+[.35,.76,.28,.10],'','','','conn(''gui_setup'',4);');
						CONN_h.menus.m_setup_00{5}=conn_menu('image',boffset+[.36,.23,.25,.53]);
                        CONN_h.menus.m_setup_00{6}=conn_menu('popup',boffset+[.38,.18,.21,.045],'',{'Display structural volume','Display structural surface'},'select display view (surface view only available for freesurfer-generated files)','conn(''gui_setup'',6);');
                        set([CONN_h.menus.m_setup_00{4} CONN_h.menus.m_setup_00{6}],'visible','off'); conn_menumanager('onregion',[CONN_h.menus.m_setup_00{4} CONN_h.menus.m_setup_00{6}],1,boffset+[.35,.18,.28,.70]);
...
%                                 CONN_x.Setup.normalized=normalized;
                ...
                case 4, %ROIs
                    boffset=[.06 .06 0 0];
                    if nargin<2,
                        conn_menu('frame',boffset+[.13,.03,.55,.77],'ROI data');
						...
						CONN_h.menus.m_setup_00{4}=conn_menu('pushbutton2', boffset+[.36,.62,.29,.08],'','','','conn(''gui_setup'',4);');
						CONN_h.menus.m_setup_00{5}=conn_menu('image',boffset+[.37,.17,.29,.45]);
                        set([CONN_h.menus.m_setup_00{4}],'visible','off'); conn_menumanager('onregion',[CONN_h.menus.m_setup_00{4}],1,boffset+[.36,.15,.31,.56]);
						CONN_h.menus.m_setup_00{6}=conn_menu('edit',boffset+[.38,.71,.08,.04],'ROI name','','ROI name','conn(''gui_setup'',6);');
                        fields={'Extract average timeseries','Extract PCA decomposition','Extract weighted sum timeseries'};
						CONN_h.menus.m_setup_00{7}=conn_menu('popup',boffset+[.47,.745,.20,.03],'',fields,'<HTML>Measure characterizing the ROI activation <br/> - use <i>average</i> to extract the mean BOLD timeseries within the ROI voxels <br/> - use <i>PCA</i> to extract one or several PCA components in addition to the average timeseries within the ROI voxels (e.g. for aCompCor) <br/> - use <i>weighted sum</i> to extract a weighted sum timeries within the ROI voxels (voxels are weighted by ROI mask values) (e.g. for dual-regression or probabilistic ROI definitions)</HTML>','conn(''gui_setup'',7);');
                        str=arrayfun(@(n)sprintf('from functional set %d',n),0:max(numel(CONN_x.Setup.roifunctional),max(CONN_x.Setup.rois.unsmoothedvolumes)),'uni',0);
						CONN_h.menus.m_setup_00{13}=conn_menu('popup0',boffset+[.472,.71,.20,.03],'',str,'<HTML>source of functional data for ROI timeseries extraction<br/> - Select <b>set 1 or above</b> to extract ROI BOLD timeseries from the funcional volumes defined in the tab <i>Setup.Functional.Functional data for <b>ROI</b>-level analyses</i> (default behavior; e.g. unsmoothed volumes)<br/> - Select <b>set 0</b> to extract ROI BOLD timeseries from the functional volumes defined in the tab <i>Setup.Functional.Functional data for <b>voxel</b>-level analyses</i> (non-default behavior; e.g. smoothed volumes)</HTML>','conn(''gui_setup'',13);');
						...
						[CONN_h.menus.m_setup_00{12},CONN_h.menus.m_setup_00{20}]=conn_menu('checkbox',boffset+[.50,.080,.02,.03],'Regress-out covariates','','<HTML>regress out covariates before performing PCA decomposition of BOLD signal within ROI<br/> - this field only applies when extracting more than 1 dimension (<i>extract PCA decomposition</i> option) from an ROI</HTML>','conn(''gui_setup'',12);');
                        ...
                        CONN_h.menus.m_setup_00{14}=conn_menu('popup0',boffset+[.14,.03,.20,.05],'',{'<HTML><i> - ROI tools:</i></HTML>','Slice viewer','Slice viewer with structural overlay','Check ROI/functional coregistration','Check ROI/anatomical coregistration','Check ROI labels','Add ICA network-ROIs'},'<HTML> - <i>slice viewer</i> displays ROI slices<br/>  - <i>check registration</i> checks the coregistration of the selected subject ROI and anatomical/functional files<br/> - <i>check ROI labels</i> displays ROI labels for ROI atlas files and allows editing the associated labels file<br/> <i>Add ICA network-ROIs</i> adds ICA networks (spatial components from group-ICA analysis results) as a new ROI atlas</HTML>','conn(''gui_setup'',14);');
...
%                                 CONN_x.Setup.normalized=normalized;
                                    ...
                                    case 6, % check ROI names
                                        ...
                                        if ~isempty(filename)&&(strcmp(nameext,'.img')||strcmp(nameext,'.nii')),
                                            ...
                                            if CONN_x.Setup.rois.multiplelabels(nrois), 
                                                ...
                                                [nill,roinames]=conn_rex(filename,filename,'level',level);
                                                ...
                                                answ=questdlg({sprintf('ROI file contains %d individual ROIs with labels:',numel(roinames)),' ',roinames2{:},' ','Do you want to edit the labels file now?'},'','Yes','No','Yes');
                                                if strcmp(answ,'Yes')
                                        ...
                                        return;
                                    case 7, % adds ICA ROIs
                                        set(CONN_h.menus.m_setup_00{14},'value',1);
                                        filename=fullfile(CONN_x.folders.firstlevel_vv,['ICA.ROIs.nii']);
                                        if isempty(dir(filename)), uiwait(warndlg({'ICA results not found',' ','Please run first group-ICA anayses (from first-level analysis voxel-to-voxel tab)'},'')); 
                                        else
                                            nrois=numel(CONN_x.Setup.rois.names);
                                            if any(strcmp(CONN_x.Setup.rois.names,'group-ICA'))
                                                set(CONN_h.menus.m_setup_00{1},'value',nrois);
                                            else
                                                CONN_x.Setup.rois.names{nrois}='group-ICA';
                ...
                case 8, % options
                    ...
                    if nargin<2,
                        ...
                        analysistypes={'Parametric and non-parametric analyses','Only parametric statistics, pooled variance model (Random Field Theory)','Only non-parametric statistics, multivariate model (permutation tests)'};
                        CONN_h.menus.m_setup_00{6}=conn_menu('popup',boffset+[.2,.3,.35,.05],'Second-level analyses (voxel-level)',analysistypes,'<HTML>Choose type of second-level voxel-level analyses<br/> <i> - parametric statistics, pooled variance model</i> uses parametric distributions (Random Field Theory) for cluster-level statistics, assumes<br/>similar between-conditions or between-sources covariance across voxels (equal up to scaling factor; estimated using SPM''s ReML) and <br/> similar spatial covariance across voxels (equal up to scaling factor; estimated using SPM''s residual smoothness estimation) . The analysis <br/>results can be exported to SPM software<br/> - <i>non-parametric statistics, multivariate model</i> uses non-parametric analyses (residual permutation/randomization tests) for cluster level <br/>statistics, and multivariate between-conditions or between-sources covariance estimation (multivariate analyses separately for each voxel).<br/> This provides additional control in cases where the assumptions of RFT might not be met, but the statistics will take longer to compute and<br/> the results cannot be exported to SPM software<br/> - <i>Parametric and non-parametric analyses</i> will perform both types of analyses (in the second-level <i>results explorer</i> window switch between the <i>parametric</i> and <i>non-parametric</i> options<br/> to explore both results</HTML>','conn(''gui_setup'',6);');
                        analysistypes={'PSC (percent signal change)','Raw'};
                        CONN_h.menus.m_setup_00{5}=conn_menu('popup',boffset+[.2,.2,.25,.05],'Analysis units',analysistypes,'Choose BOLD signal units for analyses','conn(''gui_setup'',5);');
                        ...
                        CONN_h.menus.m_setup_00{4}=conn_menu('checkbox2',boffset+[.5,.75,.25,.04],'Optional output files',analysistypes,'Choose optional output files to be generated during the analyses','conn(''gui_setup'',4);');
...
%             end
		...
		case 'gui_setup_finish',
			if isempty(CONN_x.filename), conn gui_setup_save; end
            if conn_questdlg('Ready to run Setup pipeline',[],CONN_x.Setup.steps(1:3),false,[],true,[],true);
                conn_menumanager clf;
                conn_menuframe;
                conn_menumanager([CONN_h.menus.m_setup_06,CONN_h.menus.m0],'on',1);
                if CONN_x.gui.parallel~=0, 
...
%                     dof=CONN_h.menus.m_preproc.opt.dof;
                    ...
                    if isfield(CONN_h.menus.m_preproc.X1,'samplexyz'), xyz=cell2mat(CONN_h.menus.m_preproc.X1.samplexyz);
                    ...
                    fy=mean(abs(fy(1:round(size(fy,1)/2),:)),2); dof=max(0,sum(fy)^2/sum(fy.^2)-size(xf,2)); % change dof displayed to WelchSatterthwaite residual dof approximation
		...
		case 'gui_preproc_done',
			if isempty(CONN_x.filename), conn gui_setup_save; end
            if conn_questdlg('Ready to run Denoising pipeline',[],CONN_x.Setup.steps(1:3),[],[],true,[],true);
                conn_menumanager clf;
                conn_menuframe;
                conn_menumanager([CONN_h.menus.m_setup_06,CONN_h.menus.m0],'on',1);
                if CONN_x.gui.parallel~=0, 
		...
		case 'gui_analyses',
            ...
            if state(1)==1, %SEED-TO-VOXEL or ROI-TO-ROI
                ...
                if nargin<2,
                    ...
                    CONN_h.menus.m_analyses_00{20}=conn_menu('popup2',[.005,.88,.145,.04],'Analysis name:',txt(ianalysis,:),'<HTML>Analysis name <br/> - Select existing first-level analysis set to edit its properties <br/> - select <i>new</i> to define a new set of first-level analyses within this project</HTML>','conn(''gui_analyses'',20);');
                    ...
                    CONN_h.menus.m_analyses_00{11}=conn_menu('listbox',boffset+[.90,.43,.075,.22],'Subjects','','Select subject to display','conn(''gui_analyses'',11);');
                    CONN_h.menus.m_analyses_00{12}=conn_menu('listbox',boffset+[.90,.13,.075,.22],'Conditions','','Select condition to display','conn(''gui_analyses'',12);');
                    ...
                    CONN_h.menus.m_analyses_00{13}=conn_menu('popup',boffset+[.65,.80,.20,.04],'',{' TOTAL'},'Select source to display','conn(''gui_analyses'',13);');
                    pos=[.56,.10,.30,.66];
                    if any(CONN_x.Setup.steps([2,3])),
                    ...
                    switch(varargin{2}),
                        ...
                        case 10,
                            ...
                            elseif value==3 % temporal modulation dynamic FC
                                filename=fullfile(CONN_x.folders.firstlevel_dyn,['dyn_Subject',num2str(1,'%03d'),'.mat']);
                                try
                                    load(filename,'names');
...
%                                 wx=CONN_h.menus.m_analyses.ConditionWeights{nsubs,3}(:,[setdiff(validconditions,nconditions) nconditions]); %gPPI
                            elseif ~isempty(regexp(CONN_x.Analyses(ianalysis).modulation,'^Dynamic factor \d+$')), 
                                filename=fullfile(CONN_x.folders.firstlevel_dyn,['dyn_Subject',num2str(nsubs,'%03d'),'.mat']);
                                xmod=load(filename);
            ...
            elseif state(1)==3 % VOXEL-TO-VOXEL
                ...
                if nargin<2,
                    ...
                    [CONN_h.menus.m_analyses_00{7}(1),CONN_h.menus.m_analyses_00{7}(2)]=conn_menu('edit',boffset+[.42,.7,.11,.04],'Analysis name','','name of voxel-to-voxel analysis','conn(''gui_analyses'',7);');
                    [CONN_h.menus.m_analyses_00{4}(1),CONN_h.menus.m_analyses_00{4}(2)]=conn_menu('edit',boffset+[.42,.6,.11,.04],'Kernel size (mm)','','<HTML>Define local kernel size (FWHM of Gaussian kernel) <br/> - special cases: <i>0</i> for single voxel; <i>inf</i> for entire brain</HTML>','conn(''gui_analyses'',4);');
                    ...
                    [CONN_h.menus.m_analyses_00{9}(1),CONN_h.menus.m_analyses_00{9}(2)]=conn_menu('edit',boffset+[.42,.6,.11,.04],'Number of factors','','<HTML>Define number of group-level components to estimate. <br/> - group-PCA analyses: Number of components retained from a Principal Component decomposition of the spatiotemporal BOLD signal (temporally-concatenated across subjects)<br/> - group-ICA: Same as <i>group-PCA</i> but with an additional Independent Components Analysis step (FastICA) to estimate spatially-independent networks<br/> - group-MVPA: Number of components retained from a Principal Component decomposition of the between-subjects variability in seed-to-voxel connectivity maps (separately for each voxel)<br/>After group-level components are computed, subject- and condition- specific components are estimated using dual regression</HTML>','conn(''gui_analyses'',9);');
                    [CONN_h.menus.m_analyses_00{8}(1),CONN_h.menus.m_analyses_00{8}(2)]=conn_menu('edit',boffset+[.42,.4,.11,.04],'Dimensionality reduction','','<HTML>(optional) subject-level dimensionality reduction step <br/> - define number of SVD components characterizing each subject Voxel-to-Voxel correlation matrix to retain <br/> - set to <i>inf</i> for no dimensionality reduction</HTML>','conn(''gui_analyses'',8);');
                    ...
                    CONN_h.menus.m_analyses_00{17}=[];[CONN_h.menus.m_analyses_00{17}(1),CONN_h.menus.m_analyses_00{17}(2)]=conn_menu('checkbox',boffset+[.42,.5,.02,.04],'Centering','','<HTML>(optional) centers MVPA components<br/> - When selecting this option the MVPA components have zero mean across all subjects/conditions (MVPA components defined using PCA of covariance in seed-to-voxel connectivity values across subjects) <br/> - Uncheck this option to skip centering (PCA decomposition uses second moment about zero instead of second moment about the mean) </HTML>','conn(''gui_analyses'',17);');
                    ...
                    if (isfield(CONN_h.menus.m_analyses.Y,'issurface')&&CONN_h.menus.m_analyses.Y.issurface)
                        ...
                        CONN_h.menus.m_analyses_00{11}=conn_menu('listbox',boffset+[.90,.53,.075,.22],'Subjects','','Select subject to display','conn(''gui_analyses'',11);');
                        CONN_h.menus.m_analyses_00{12}=conn_menu('listbox',boffset+[.90,.23,.075,.22],'Conditions','','Select condition to display','conn(''gui_analyses'',12);');
                        CONN_h.menus.m_analyses_00{13}=conn_menu('popup',boffset+[.69,.78,.18,.05],'',{' '},'Select measure to display','conn(''gui_analyses'',13);');
                        pos=[.56,.15,.30,.63];
...
%                 else  conn_menu('update',CONN_h.menus.m_analyses_00{3},CONN_h.menus.m_analyses.X); end
                if ~isempty(nregressors)&&all(nregressors>0),
                    ...
                    if ~all(temp>1), set([CONN_h.menus.m_analyses_00{9}],'visible','off'); end
                else
...
%                                 end
                ...
                if nargin<2, 
                    ...
                    conn_menu('frame',boffset+[.095,.02,.40,.75],'');%'DYNAMIC CONNECTIVITY ANALYSIS');
                    [nill,CONN_h.menus.m_analyses_00{16}]=conn_menu('text',boffset+[.105,.65,.26,.04],'Dynamic/temporal modulation of functional connectivity:');
                    set(CONN_h.menus.m_analyses_00{16},'horizontalalignment','left');
                    CONN_h.menus.m_analyses_00{4}=[]; [CONN_h.menus.m_analyses_00{4}(1),CONN_h.menus.m_analyses_00{4}(2)]=conn_menu('edit',boffset+[.105,.6,.20,.04],'Number of factors','','<HTML>Define number of group-level components to estimate <br/> - These analyses estimate a set of components characterizing independent circuits of connections that share the same modulation patterns <br/> - Components are estimated using iterative dual-regression on a gPPI model with unknown temporal-modulation terms, followed by <br/>Independent Component Analyses (FastICA) to estimate spatially-independent circuits</HTML>','conn(''gui_analyses'',4);');
                    CONN_h.menus.m_analyses_00{6}=conn_menu('edit',boffset+[.105,.52,.12,.04],'Smoothing kernel','','<HTML>Temporal modulation smoothing kernel FWHM (in seconds)</HTML>','conn(''gui_analyses'',6);');
                    CONN_h.menus.m_analyses_00{5}=conn_menu('popup',boffset+[.105,.44,.12,.04],'Condition',CONN_x.Setup.conditions.names(1:end-1),'<HTML>Determine the span of the BOLD timeseries used in these analyses <br/> - Select ''rest'' condition to use entire BOLD timeseries</HTML>','conn(''gui_analyses'',5);');
                    ...
                    CONN_h.menus.m_analyses_00{11}=[]; [CONN_h.menus.m_analyses_00{11}(1),CONN_h.menus.m_analyses_00{11}(2)]=conn_menu('checkbox',boffset+[.255,.50,.02,.03],'Export modulatory timeseries','','Export the estimated modulatory timeseries as first-level covariates for additional analyses','conn(''gui_analyses'',11);');
                    [nill,CONN_h.menus.m_analyses_00{16}]=conn_menu('text',boffset+[.105,.30,.26,.05],'ROI-to-ROI seeds/sources:');
                    set(CONN_h.menus.m_analyses_00{16},'horizontalalignment','left');
                    CONN_h.menus.m_analyses_00{1}=conn_menu('listbox',boffset+[.105,.05,.145,.25],'all ROIs','','List of all seeds/ROIs','conn(''gui_analyses'',1);');
                    CONN_h.menus.m_analyses_00{2}=conn_menu('listbox',boffset+[.27,.05,.195,.25],'Sources','','<HTML>List of seeds/ROIs to be included in this analysis  <br/> - This analysis will explore the ROI-to-ROI connectivity matrix between the selected sources for dynamic connectivity effects (temporal modulation of ROI-to-ROI connectivity values)<br/> - Select ROIs in the <i>all ROIs</i> list and click <b> > </b> to add new sources to this list<br/> - Select sources in this list and click <b> &lt </b> to remove them from this list </HTML>','conn(''gui_analyses'',2);');
                    ...
                    set([CONN_h.menus.m_analyses_00{1},CONN_h.menus.m_analyses_00{2}],'value',[]);
                    set(CONN_h.menus.m_analyses_00{4}(1),'string',num2str(CONN_x.dynAnalyses.Ncomponents));
                    set(CONN_h.menus.m_analyses_00{5},'value',CONN_x.dynAnalyses.condition);
                    set(CONN_h.menus.m_analyses_00{6},'string',mat2str(CONN_x.dynAnalyses.window));
                    %set(CONN_h.menus.m_analyses_00{12},'value',CONN_x.dynAnalyses.analyses);
                    ...
                    switch(varargin{2}),
                        ...
                        case 6,
                            ...
                            if numel(temp)==1||isempty(temp), CONN_x.dynAnalyses.window=temp; end
                            set(CONN_h.menus.m_analyses_00{6},'string',mat2str(CONN_x.dynAnalyses.window));
                        case 9,
...
%                             CONN_x.dynAnalyses.analyses=val;
                    end
                end
                nregressors=get(CONN_h.menus.m_analyses_00{2},'value');
                set(CONN_h.menus.m_analyses_00{4}(1),'string',num2str(CONN_x.dynAnalyses.Ncomponents));
                %if CONN_x.dynAnalyses.analyses==1, set([CONN_h.menus.m_analyses_00{4} CONN_h.menus.m_analyses_00{9} CONN_h.menus.m_analyses_00{10} CONN_h.menus.m_analyses_00{11}],'visible','off');
		...
		case 'gui_analyses_done',
            ...
            if conn_questdlg('Ready to run First-level Analysis processing pipeline',false,tsteps,condsoption,[],true,[],true);
                conn_menumanager clf;
                conn_menuframe;
                conn_menumanager([CONN_h.menus.m_setup_06,CONN_h.menus.m0],'on',1);
                if CONN_x.gui.parallel~=0, 
                    ...
                    if isfield(CONN_x.gui,'subjects'), subjects=CONN_x.gui.subjects; else subjects=[]; end
                    conn save;
                    conn_jobmanager('submit','analyses_gui_seed',subjects,[],CONN_x.gui,CONN_x.Analysis);
		...
		case 'gui_analyses_done_vv',
			if isempty(CONN_x.filename), conn gui_setup_save; end
            tsteps=[0 0 1]; 
            if conn_questdlg('Ready to run Voxel-to-Voxel processing pipeline',false,tsteps,[],[],true,[],true);
                conn_menumanager clf;
                conn_menuframe;
                conn_menumanager([CONN_h.menus.m_setup_06,CONN_h.menus.m0],'on',1);
                if CONN_x.gui.parallel~=0, 
                    ...
                    if isfield(CONN_x.gui,'subjects'), subjects=CONN_x.gui.subjects; else subjects=[]; end
                    conn save;
                    conn_jobmanager('submit','analyses_gui_vv',subjects,[],CONN_x.gui);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%			
        ...
        case {'gui_results','gui_results_dyn','gui_results_dyn_loadings','gui_results_dyn_scores','gui_results_dyn_factors'}
            ...
            if state==4||strcmp(lower(varargin{1}),'gui_results_dyn')||strcmp(lower(varargin{1}),'gui_results_dyn_loadings')||strcmp(lower(varargin{1}),'gui_results_dyn_scores')||strcmp(lower(varargin{1}),'gui_results_dyn_factors')
                ...
                if strcmp(lower(varargin{1}),'gui_results_dyn_loadings')
                ...
                elseif strcmp(lower(varargin{1}),'gui_results_dyn_scores')
                    ...
                    conn_menumanager(CONN_h.menus.m_results_03a,'state',tstate);
                    tstate=2;
                elseif strcmp(lower(varargin{1}),'gui_results_dyn_factors')
            ...
            if nargin<2,
                ...
                if state==1||state==2
                    ...
                    CONN_h.menus.m_results_00{20}=conn_menu('popup2',[.005,.88,.165,.04],'Analysis name:',txt(CONN_h.menus.m_results.shownanalyses),'<HTML>First-level analysis name <br/> - Select first-level analysis set</HTML>','conn(''gui_results'',20);');
				...
				if (state==1||state==2) && (~isfield(CONN_x.Analyses(ianalysis),'sources')||isempty(CONN_x.Analyses(ianalysis).sources)), uiwait(errordlg('Run first the first-level Analysis step by pressing "Done" in the Seed-to-Voxel/ROI-to-ROI first-level Analysis tab','Data not prepared for analyses')); return; end %conn gui_analyses; return; end
				if (state==3) && (~isfield(CONN_x.vvAnalyses,'measures')||isempty(CONN_x.vvAnalyses.measures)), uiwait(errordlg('Run first the first-level Analysis step by pressing "Done" in the Voxel-to-Voxel first-level Analysis tab','Data not prepared for analyses')); return; end %conn gui_analyses; return; end
				if (state==4) && (~isfield(CONN_x.dynAnalyses,'sources')||isempty(CONN_x.dynAnalyses.sources)), uiwait(errordlg('Run first the first-level Analysis step by pressing "Done" in the first-level Dynamic FC tab','Data not prepared for analyses')); return; end %conn gui_analyses; return; end
                ...
                if state==4
                    ...
                    if state==1, conn_menu('frame',boffset+[.095,.41,.45,.48],'');%'2^n^d-level analysis');
                    else conn_menu('frame',boffset+[.095,.13,.45,.75],'');%'2^n^d-level analysis');
                    end
                    
                    conn_menu('frame2',boffset+[.655,.08,.28,.85],'');%'2^n^d-level analysis');
                    CONN_h.menus.m_results_00{11}=conn_menu('listbox',boffset+[.105,.53-dp1,.145,.30+dp1],'Subject effects','','select subject effect(s) characterizing second-level analysis model','conn(''gui_results'',11);');
                    CONN_h.menus.m_results_00{16}=conn_menu('edit',boffset+[.105,.44-dp1,.145,.04],'Between-subjects contrast',num2str(1),['<HTML>Define desired contrast across selected subject-effects<br/> - enter contrast vector/matrix with as many elements/columns as subject-effects selected <br/> - ',CONN_gui.rightclick,'-click or use the list below to see a list of standard contrasts for the selected subject-effects <br/> - enter multiple rows separated by <b>;</b> (semicolon) for OR conjunction (multivariate test) of several contrasts</HTML>'],'conn(''gui_results'',16);');
                    CONN_h.menus.m_results_00{12}=conn_menu('listbox',boffset+[.250,.53-dp1,.145,.30+dp1],'Conditions','','select condition(s) of interest','conn(''gui_results'',12);');
                    CONN_h.menus.m_results_00{19}=conn_menu('edit',boffset+[.250,.44-dp1,.145,.04],'Between-conditions contrast',num2str(1),['<HTML>Define desired contrast across selected conditions <br/> - enter contrast vector/matrix (as many elements/columns as conditions selected) <br/> - ',CONN_gui.rightclick,'-click or use the list below to see a list of standard contrasts for the selected conditions<br/> - enter multiple rows separated by <b>;</b> (semicolon) for OR conjunction (multivariate test) of several contrasts</HTML>'],'conn(''gui_results'',19);');
                    %connmeasures={'correlation (bivariate)','correlation (semipartial)','regression (bivariate)','regression (multivariate)'};
                    if state==3, 
                        CONN_h.menus.m_results_00{13}=conn_menu('listbox',boffset+[.395,.53-dp1,.145,.30+dp1],'Voxel-to-Voxel Measures','','select voxel-to-voxel measure(s) of interest','conn(''gui_results'',13);');
                        CONN_h.menus.m_results_00{17}=conn_menu('edit',boffset+[.395,.44-dp1,.145,.04],'Between-measures contrast',num2str(1),['<HTML>Define desired contrast across selected measures <br/> - enter contrast vector/matrix (as many elements/columns as measures selected) <br/> - ',CONN_gui.rightclick,'-click or use the list below to see a list of standard contrasts for the selected measures<br/> - enter multiple rows separated by <b>;</b> (semicolon) for OR conjunction (multivariate test) of several contrasts</HTML>'],'conn(''gui_results'',17);');
                    else
                        CONN_h.menus.m_results_00{13}=conn_menu('listbox',boffset+[.395,.53-dp1,.145,.30+dp1],'Seeds/Sources','','select seed/source ROI(s) of interest','conn(''gui_results'',13);');
                        CONN_h.menus.m_results_00{17}=conn_menu('edit',boffset+[.395,.44-dp1,.145,.04],'Between-sources contrast',num2str(1),['<HTML>Define desired contrast across selected sources <br/> - enter contrast vector/matrix (as many elements/columns as sources selected) <br/> - ',CONN_gui.rightclick,'-click or use the list below to see a list of standard contrasts for the selected sources<br/> - enter multiple rows separated by <b>;</b> (semicolon) for OR conjunction (multivariate test) of several contrasts</HTML>'],'conn(''gui_results'',17);');
                    end
                    
                    if state==2||state==3,
                        pos=[.58,.14,.335,.73];
                        if ~isfield(CONN_x.Results.xX,'displayvoxels'), CONN_x.Results.xX.displayvoxels=1; end
                        CONN_h.menus.m_results_00{24}=uicontrol('style','text','units','norm','position',boffset+[pos(1)+pos(3)-.195,pos(2)-1*.05,.095,.035],'string','p-uncorrected <','fontname','default','fontsize',8+CONN_gui.font_offset,'backgroundcolor',CONN_gui.backgroundcolorA,'foregroundcolor',CONN_gui.fontcolorA);
                        ...
                        CONN_h.menus.m_results_00{32}=conn_menu('popup',boffset+[.67,.87,.19,.045],'',strstr3,'Display options','conn(''gui_results'',32);');
                    ...
                    if state==1 % ROI-to-ROI
                        [CONN_h.menus.m_results_00{18},CONN_h.menus.m_results_00{22}]=conn_menu('listbox2',boffset+[.105,.07,.435,.23],sprintf('%-50s%10s%10s%12s%12s','Targets','beta','T','p-unc','p-FDR'),'   ','browse target ROIs -or right click for more options-','conn(''gui_results'',18);');
                        ...
                        CONN_h.menus.m_results_00{29}=conn_menu('popup',boffset+[.64,.10,.10,.04],'',strstr2,'choose type of false-positive control','conn(''gui_results'',29);');
                        set(CONN_h.menus.m_results_00{29},'value',CONN_x.Results.xX.inferenceleveltype);
                        CONN_h.menus.m_results_00{30}=conn_menu('edit',boffset+[.74,.10,.04,.04],'',num2str(CONN_x.Results.xX.inferencelevel),'enter false-positive threshold value','conn(''gui_results'',30);');
                        CONN_h.menus.m_results_00{28}=conn_menu('popup',boffset+[.79,.10,.09,.04],'',strstr1,'choose inference directionality','conn(''gui_results'',28);');
                        set(CONN_h.menus.m_results_00{28},'value',CONN_x.Results.xX.inferencetype);
                        CONN_h.menus.m_results_00{31}=conn_menu('popup',boffset+[.66,.87,.20,.04],'',strstr3,'choose target ROIs','conn(''gui_results'',31);');
                        ...
                        CONN_h.menus.m_results_00{25}=conn_menu('axes',boffset+[.57,.15,.34,.72]);
                        ...
                        %xs=conn_conv(mean(CONN_gui.refs.canonical.data.^2,3)',[0,0]);xs=max(0,min(1,xs/max(xs(:))-.1))/8;
                        ...
                        hold off;
                        CONN_h.menus.m_results_00{33}=conn_menu('pushbutton',boffset+[.60,.05,.08,.045],'','display 3D','displays 3d view of current analysis results','conn(''gui_results_roi3d'');');
                        CONN_h.menus.m_results_00{34}=conn_menu('pushbutton',boffset+[.72,.05,.08,.045],'','display values','<HTML>display effet sizes between selected ROI pairs for each condition/source <br/> - also exports values for each subject to Matlab workspace</HTML>','conn(''gui_results'',35);');
                        CONN_h.menus.m_results_00{35}=conn_menu('pushbutton',boffset+[.84,.05,.08,.045],'','import values','import connectivity values between selected ROI pairs for each condition/source/subject as 2nd-level covariates','conn(''gui_results'',34);');
                        set([CONN_h.menus.m_results_00{33},CONN_h.menus.m_results_00{34},CONN_h.menus.m_results_00{35}],'visible','off');%,'fontweight','bold');
                        conn_menumanager('onregion',[CONN_h.menus.m_results_00{33},CONN_h.menus.m_results_00{34},CONN_h.menus.m_results_00{35}],1,boffset+[.545,.05,.38,.84]);
                    end
                    ...
                    if state==1||state==2
                        ...
                        filepathresults=CONN_x.folders.firstlevel_vv;
                    end
...
%                     end
                    ...
                    if numel(CONN_x.Setup.l2covariates.names)>2||numel(CONN_x.Setup.conditions.names)>2
                        ...
                        CONN_h.menus.m_results_00{21}=conn_menu('popup2',[.005,.84,.125,.04],'',[{'user-defined contrast'},tnames,{'<HTML><i> save/delete contrast</i></HTML>'}],'User-defined list of contrasts of interest (between-subjects&conditions)','conn(''gui_results'',21);');
...
%                 end
            ...
            if state==1, % ROI-level
                ...
                if ~roierr
                    ...
                    if ~isempty(ntemp2)
                        ...
                        for niplot=8:-1:1, CONN_h.menus.m_results_00{26}=cat(2,CONN_h.menus.m_results_00{26},patch(conn_bsxfun(@plus,xyz(1,idxsort),conn_bsxfun(@times,niplot/9*3*wtemp(ntemp2(idxsort)),xtemp)),conn_bsxfun(@plus,xyz(2,idxsort),conn_bsxfun(@times,niplot/9*3*wtemp(ntemp2(idxsort)),ytemp)),ones(numel(xtemp),1)*xyz(3,idxsort),'b','facecolor',min(1,4*[.25 .25 1]*(1-niplot/9)),'edgecolor','none','facealpha',.95+0*(1-niplot/9)^1)); end
                        %CONN_h.menus.m_results_00{26}=cat(2,CONN_h.menus.m_results_00{26},patch(conn_bsxfun(@plus,xyz(1,idxsort),conn_bsxfun(@times,0+3*wtemp(ntemp2(idxsort)),xtemp)),conn_bsxfun(@plus,xyz(2,idxsort),conn_bsxfun(@times,0+3*wtemp(ntemp2(idxsort)),ytemp)),'b','facecolor',[.25 .25 1],'edgecolor','none','facealpha',.75));
                    ...
                    if ~isempty(ntemp2)
                        xyz=pinv(CONN_gui.refs.canonical.V.mat)*[cat(1,CONN_h.menus.m_results.roiresults.xyz2{ntemp(ntemp2)}),ones(numel(ntemp2),1)]';
                        [nill,idxsort]=sort(xyz(3,:));
                        for niplot=8:-1:1, CONN_h.menus.m_results_00{26}=cat(2,CONN_h.menus.m_results_00{26},patch(conn_bsxfun(@plus,xyz(1,idxsort),conn_bsxfun(@times,niplot/9*3*wtemp(ntemp2(idxsort)),xtemp)),conn_bsxfun(@plus,xyz(2,idxsort),conn_bsxfun(@times,niplot/9*3*wtemp(ntemp2(idxsort)),ytemp)),ones(numel(xtemp),1)*xyz(3,idxsort),'r','facecolor',min(1,4*[1 .25 .25]*(1-niplot/9)),'edgecolor','none','facealpha',.95+0*(1-niplot/9)^1)); end
                        %CONN_h.menus.m_results_00{26}=cat(2,CONN_h.menus.m_results_00{26},patch(conn_bsxfun(@plus,xyz(1,idxsort),conn_bsxfun(@times,0+3*wtemp(ntemp2(idxsort)),xtemp)),conn_bsxfun(@plus,xyz(2,idxsort),conn_bsxfun(@times,0+3*wtemp(ntemp2(idxsort)),ytemp)),'r','edgecolor','none','facealpha',.75));
                        CONN_h.menus.m_results.roiresults.displayrois=cat(1,CONN_h.menus.m_results.roiresults.displayrois,[cat(1,CONN_h.menus.m_results.roiresults.xyz2{ntemp(ntemp2)}),2*3*wtemp(ntemp2)',ones(numel(ntemp2),1), xyz(1:2,:)', reshape(ntemp(ntemp2),[],1)]);
                        CONN_h.menus.m_results.roiresults.displayroisnames=cat(2,CONN_h.menus.m_results.roiresults.displayroisnames,CONN_h.menus.m_results.roiresults.names2(ntemp(ntemp2)));
                    ...
                    if ~isempty(ntarget)
                        for itarget=1:numel(ntarget)
                            ...
                            if nnz(~isnan(cbeta))
                                ...
                                h0a=line(xyz(1)-2+1*xrange*(size(cbeta,1))/2*[-1 1],xyz(2)+2+wt2+0.125*10+10*(-crange(1)/diff(crange))+[0 0],200+numel(ntarget)-itarget+1+[.2 .2],'color','k','linewidth',1);
                                h0b=patch(xyz(1)-2+1.25*xrange*(size(cbeta,1))/2*[-1 -1 1 1],xyz(2)+2+wt2+1.25*10*[0 1 1 0],200+numel(ntarget)-itarget+1+[0 0 0 0],'k','facecolor','w','edgecolor','none','linestyle','-','linewidth',1,'facealpha',.90);
                                h0c=patch(xyz(1)+[0 -2 -.5],xyz(2)+[0 2 2]+wt2,200+numel(ntarget)-itarget+1+[.2 .2 .2],'w','facecolor','w','edgecolor','none','facealpha',.90);
                                [h1,h2]=conn_plotbars(cbeta,CI, [xyz(1)-2+xrange*size(cbeta,1)/2+xrange/2, xyz(2)+2+wt2+0.125*10+10*(-crange(1)/diff(crange)), 200+numel(ntarget)-itarget+1, -xrange, 10/diff(crange), .1]);
                    ...
                    if ~isempty(nsources)
                        ...
                        [nill,idxsort]=sort(xyz(3,:));
                        for niplot=8:-1:1, CONN_h.menus.m_results_00{26}=cat(2,CONN_h.menus.m_results_00{26},patch(conn_bsxfun(@plus,xyz(1,idxsort),niplot/9*4*xtemp),conn_bsxfun(@plus,xyz(2,idxsort),niplot/9*4*ytemp),ones(numel(xtemp),1)*xyz(3,idxsort),'w','facecolor',min(1,4*[.25 .25 .25]*(1-niplot/9)),'edgecolor','none','facealpha',.95+0*(1-niplot/9)^1)); end
        ...
        case 'gui_results_roi3d'
            ...
            conn_mesh_display('','',[],struct('sph_names',{CONN_h.menus.m_results.roiresults.displayroisnames},'sph_xyz',CONN_h.menus.m_results.roiresults.displayrois(:,1:3),'sph_r',CONN_h.menus.m_results.roiresults.displayrois(:,4),'sph_c',{c(2+sign(CONN_h.menus.m_results.roiresults.displayrois(:,5)))}), [], .2, [0,-.01,1]);
            return
...
function ok=conn_questdlg(str,stepsoption,steps,condsoption,dispoption,paroption,multipleoption,subjectsoption)
...
ht4b=uicontrol('style','listbox','units','norm','position',[.7,.6,.2,.2],'string',CONN_x.Setup.conditions.names(1:end-1),'value',1,'fontsize',8+CONN_gui.font_offset,'max',2);
ht4a=uicontrol('style','checkbox','units','norm','position',[.5,.7,.19,.10],'string','All conditions','value',all(islogical(condsoption)),'fontsize',8+CONN_gui.font_offset,'backgroundcolor','w','userdata',ht4b,'callback','h=get(gcbo,''userdata'');if get(gcbo,''value''), set(h,''visible'',''off''); else set(h,''visible'',''on''); end');
ht4d=uicontrol('style','listbox','units','norm','position',[.7,.6,.2,.2],'string',{'Setup step','Denoising step','First-level analysis step'},'value',1,'fontsize',8+CONN_gui.font_offset,'max',2);
ht4c=uicontrol('style','checkbox','units','norm','position',[.5,.7,.19,.10],'string','All Steps','value',0,'fontsize',8+CONN_gui.font_offset,'backgroundcolor','w','userdata',ht4d,'callback','h=get(gcbo,''userdata'');if get(gcbo,''value''), set(h,''visible'',''off''); else set(h,''visible'',''on''); end');
ht4f=uicontrol('style','listbox','units','norm','position',[.7,.4,.2,.2],'string',arrayfun(@(x)sprintf('Subject %d',x),1:CONN_x.Setup.nsubjects,'uni',0),'value',1,'fontsize',8+CONN_gui.font_offset,'max',2);
ht4e=uicontrol('style','checkbox','units','norm','position',[.5,.5,.19,.10],'string','All Subjects','value',all(islogical(subjectsoption)),'fontsize',8+CONN_gui.font_offset,'backgroundcolor','w','userdata',ht4f,'callback','h=get(gcbo,''userdata'');if get(gcbo,''value''), set(h,''visible'',''off''); else set(h,''visible'',''on''); end');
