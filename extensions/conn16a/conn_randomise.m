...
if nargin<11||isempty(datatype), datatype='matrix'; end % 'matrix'|'surface'|'volume'
...
Hist_Cluster_mass=[Hist_Cluster_mass,repmat({sparse(1+maxT*Nr,1)},1,nthr)];
...
Hist_Seed_mass=[Hist_Seed_mass,repmat({sparse(1+maxT*Nr,1)},1,nthr)];
...
if isdisplay, ht=conn_timedwaitbar(0,'Updating non-parametric statistics. Please wait'); 
...
for niter=1:niters
    ...
    for i=thridx % all new threshold values
        ...
        Hist_Cluster_mass{i}=Hist_Cluster_mass{i}+sparse(1+round(min(maxT*Nr,maxT*mclL)),1,1/numel(mclL),maxT*Nr+1,1);   % histogram of cluster mass (note: Hist_(1) corresponds to cluster mass=0)
        ...
        if ischar(datatype)&&strcmp(datatype,'matrix') % additional stats for ROI-to-ROI analyses
            ...
            Hist_Seed_mass{i}=Hist_Seed_mass{i}+sparse(1+round(min(maxT*Nr,maxT*msdL)),1,1/numel(msdL),maxT*Nr+1,1);   % histogram of seed mass (note: Hist_(1) corresponds to seed mass=0)
        end
    end
