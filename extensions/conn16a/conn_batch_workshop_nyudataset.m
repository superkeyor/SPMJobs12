...
if ~nargin, 
    ...
    try
        answ=questdlg({'Preparing to download and analyze the NYU test-retest dataset (http://www.nitrc.org/projects/nyu_trt/)',' ','This procedure will create a new CONN project with the fully analyzed NYU test-retest dataset',' ','The full dataset requires approximately 200Gb of hard-drive space and 30 hours to finish. You may download/process the full dataset or only a portion of it',' ','Do you want to proceed?'},'Sample data download','Full dataset','Partial dataset','Cancel','Full dataset');
...
if UNZIPFILES
    ...
    %% UNZIP .nii.gz files
    a=strvcat(conn_dir('lfo.nii.gz'),conn_dir('mprage_anonymized.nii.gz'));
    for n1=1:size(a,1),
...
STRUCTURAL_FILE=cellstr(conn_dir('mprage_anonymized.nii'));
