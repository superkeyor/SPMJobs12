...
ht2=conn_menu('listbox',boffset+[.45 .77 .14 .10],'Components',names,'<HTML>Select factor(s) for display <br/> - <i>Factor scores</i> will display only the selected factors <br/> - <i>ROI-to-ROI connectivity</i> will display only the contribution of the selected factors on the estimated dynamic ROI-to-ROI connectivity values</HTML>',@(varargin)conn_dynexplore_update([0 0 1 0]));
ht1=conn_menu('listbox',boffset+[.45 .62 .14 .10],'Subjects',arrayfun(@(n)sprintf('Subject %d',n),1:CONN_x.Setup.nsubjects,'uni',0),'Select subject(s) for display',@(varargin)conn_dynexplore_update);
ht6a=conn_menu('popup',boffset+[.45 .54 .14 .04],'Seed ROI',ROInamesDisplay,'<HTML>Select seed ROI for dynamic connectivity timeseries display</HTML>',@(varargin)conn_dynexplore_selectroi);
ht6b=conn_menu('popup',boffset+[.45 .47 .14 .04],'Target ROI',ROInamesDisplay,'<HTML>Select target ROI for dynamic connectivity timeseries display</HTML>',@(varargin)conn_dynexplore_selectroi);
...
ht4=conn_menu('popup',boffset+[.70 .87 .20 .04],'',{'ROI-to-ROI connectivity matrix (dynamic)','Static vs. Dynamic connectivity plot'},'<HTML>Select connectivity display</HTML>',@(varargin)conn_dynexplore_update);
ht4b=conn_menu('popup',boffset+[.70 .83 .20 .04],'',{'Display full connectivity','Display selected component(s) contribution only'},'<HTML>Select full connectivity to display the full dynamic ROI-to-ROI connectivity values or <br/> only the portion explained by the selected component(s)</HTML>',@(varargin)conn_dynexplore_update);
...
set(ht6a,'value',idxlock(2));
set(ht6b,'value',idxlock(1));
...
%conn_dynexplore_play;
    ...
    function conn_dynexplore_update(doreset)
        ...
        else c=reshape(H(ntime,nfac)*B(nfac,:),size(B,2),size(B,3));
        end
        ...
        if doreset(3)||isempty(handleload)||any(~ishandle(handleload))
            ...
            hold on; h2a=text(get(gca,'xlim')*[1.05;-.05],mean(get(gca,'ylim')),{'Component','loadings'},'color',.1*[1 1 1]+.8*(.5>mean(CONN_gui.backgroundcolor)),'horizontalalignment','right','fontsize',CONN_gui.font_offset+8); hold off;
        ...
        if doreset(4)||isempty(handlescor)||any(~ishandle(handlescor))
            ...
            hold on; h2a=text(get(gca,'xlim')*[1.025;-.025],0,{'Component','timeseries'},'color',.1*[1 1 1]+.8*(.5>mean(CONN_gui.backgroundcolor)),'horizontalalignment','right','fontsize',CONN_gui.font_offset+8); 
        ...
        for n=1:Nf, if any(nfac==n), set(handlescor(n),'xdata',1:Nt,'ydata',H(itime,n)/Hscale); else set(handlescor(n),'xdata',1:Nt,'ydata',nan(1,Nt)); end; end
        if ptype==1, tempr=H0(itime,:)*B0(:,idxlock(1),idxlock(2))+H(itime,:)*B(:,idxlock(1),idxlock(2));  % complete fit
        else tempr=H0(itime,:)*B0(:,idxlock(1),idxlock(2))+H(itime,nfac)*B(nfac,idxlock(1),idxlock(2)); % only portion explained by selected factor
        end
    ...
    function conn_dynexplore_selectroi(varargin)
        idxlock(1)=get(ht6b,'value');
        idxlock(2)=get(ht6a,'value');
        idxlock(3)=1;
        conn_dynexplore_update;
    end
